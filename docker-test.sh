...
# Verify the runtime and database through the public health endpoint.
echo "Testing runtime health..."
HEALTH_RESPONSE=$(docker exec fredy curl -sf http://localhost:9998/health 2>&1)
if echo "$HEALTH_RESPONSE" | grep -q '"status":"ok"'; then
  echo "Runtime and database are healthy"
else
  echo "Runtime health check failed: $HEALTH_RESPONSE"
  docker logs fredy
  exit 1
fi
