  ...
  async _captureAndEnqueue(newListings) {
    ...
    for (const listing of listingsToCapture) {
      ...
      try {
        ...
        if (!capture) {
          logger.warn(
            `Detail capture failed for '${listing.title}' (Provider: '${this._providerId}'); queuing discovery evidence.`,
            lastError,
          );
          capture = discoveryCapture(this._providerId, listing);
        }
...
function discoveryCapture(provider, listing) {
  const fullText = [
    listing.title,
    listing.address,
    listing.description,
    listing.price != null ? `Price: ${listing.price}` : null,
    listing.size != null ? `Size: ${listing.size} m²` : null,
  ...
  return {
    ...
    fullText,
    embeddedData: [],
    images: listing.image ? [{ position: 0, kind: 'photo', originalUrl: listing.image }] : [],
