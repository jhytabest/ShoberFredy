...
class FredyPipelineExecutioner {
   ...
   * outside every polygon). The spatial check fails open when the geocoder
   * was unavailable.
   *
   * @param {ParsedListing[]} newListings
  ...
  _evaluateVisibility(newListings) {
    for (const listing of newListings) {
      listing.hidden_reason = this._visibilityVerdict(listing);
   ...
   */
  _visibilityVerdict(listing) {
    if (isOneOf(listing.title, this._jobBlacklist) || isOneOf(listing.description, this._jobBlacklist)) {
      return 'blacklist';
    }
    ...
    if (polygonFeatures?.length) {
      ...
      if (!hasValidCoordinates) {
        if (listing.geocode_unavailable) {
          ...
          logger.warn(
            `Geocoding unavailable; keeping listing visible despite missing coordinates: '${listing.title}' (Provider: '${this._providerId}')`,
          );
          return null;
        }
