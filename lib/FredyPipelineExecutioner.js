...
class FredyPipelineExecutioner {
  constructor(providerConfig, job, providerId, browser, { maxPages = null, signal } = {}) {
  ...
  async _discoverPages(baseUrl) {
    ...
    for (let page = 1; page <= maxPages; page++) {
      ...
      try {
        const discovered = await (this._providerConfig.getListings?.(url, this._browser, this._signal) ??
