...
import { crossPortalDedupe } from './services/listings/crossPortalDedupe.js';
import { decorateListingsForNotification, persistListingScores } from './services/scoring/marketScore.js';
 ...
 * 9) Score every saved listing against the persisted market model
 * 10) Filter out entries similar to already seen ones
 * 11) Suppress cross-portal duplicates of recently notified flats
 * 12) Filter out entries that do not match the job's specFilter
 * 13) Filter out entries that do not match the job's spatialFilter
 * 14) Decorate surviving listings with a market-score line (notifying jobs)
 * 15) Dispatch notifications
 */
class FredyPipelineExecutioner {
    ...
    for (const listing of newListings) {
      if (listing.address) {
        ...
        if (coords && coords.lat !== -1 && coords.lng !== -1) {
          ...
          // is unavailable (quota, denied key, transport). Mark the listing
          // so the spatial filter fails open instead of silently deleting it.
          listing.geocode_unavailable = true;
        }
      }
    }
    return newListings;
  ...
  _filterByArea(newListings) {
    ...
    const keptListings = newListings.filter((listing) => {
      ...
      if (!hasValidCoordinates) {
        if (listing.geocode_unavailable) {
          // Fail open: the geocoder was down, not the address unknown.
          logger.warn(
            `Geocoding unavailable; keeping listing despite missing coordinates: '${listing.title}' (Provider: '${this._providerId}')`,
          );
          return true;
        }
        logger.debug(
          `Filtering listing without valid coordinates for spatial filter: '${listing.title}' (Provider: '${this._providerId}')`,
        );
  ...
  _save(newListings) {
    ...
    this._hideShadowJobListings(newListings);
    return newListings;
  }

  /**
   * Shadow jobs (no notification adapters) exist only to feed the market
   * model an unfiltered corpus; their rows must never surface in the UI or
   * be matched by the cross-portal dedupe. Soft-hide them immediately after
   * save. (Replaces the shadow_jobs_soft_hide_after_insert DB trigger, which
   * hardcoded job ids; this applies to any adapterless job.)
   *
   * @param {ParsedListing[]} newListings Freshly stored listings.
   * @returns {void}
   */
  _hideShadowJobListings(newListings) {
    if (Array.isArray(this._jobNotificationConfig) && this._jobNotificationConfig.length > 0) {
      return;
    }
    const ids = newListings.map((listing) => listing.id).filter((id) => id != null);
    if (ids.length === 0) {
      return;
    }
    logger.debug(`Soft-hiding ${ids.length} shadow-job listings (Provider: '${this._providerId}')`);
    deleteListingsById(ids);
  }

  /**
   * Score every stored listing against the persisted market model directly
   * after save, so main and shadow jobs alike get a market score the moment
   * a listing enters the database. Fails open.
   *
   * @param {ParsedListing[]} newListings
   * @returns {ParsedListing[]} The same listings, unchanged.
   */
  _persistScores(newListings) {
    return persistListingScores(newListings);
  }

  /**
   * Suppress cross-portal duplicates of flats already covered by a visible
   * listing of a notifying job within the last 7 days. Runs directly after
   * Fredy's own similarity dedupe so all duplicate suppression happens in
   * one place. Shadow jobs are unaffected.
   *
   * @param {ParsedListing[]} newListings
   * @returns {ParsedListing[]} Listings that are not cross-portal duplicates.
   */
  _crossPortalDedupe(newListings) {
    return crossPortalDedupe(newListings, {
      providerId: this._providerId,
      notificationAdapters: this._jobNotificationConfig,
    });
  }

  /**
   * Append the market-score metrics line to each listing right before
   * notification, so the line is only rendered for listings that survived
   * every filter. The listings are already persisted, so the decoration
   * never reaches the listings table.
   *
   * @param {ParsedListing[]} newListings
   * @returns {ParsedListing[]} The same listings, decorated in place.
   */
  _decorateForNotification(newListings) {
    return decorateListingsForNotification(newListings, {
      notificationAdapters: this._jobNotificationConfig,
    });
  }
