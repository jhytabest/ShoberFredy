  ...
  async _discoverPages(baseUrl) {
    ...
    for (let page = 1; page <= maxPages; page++) {
      ...
      for (const listing of listings) {
        // One card per attempt, because this loop had no guard at all: a throw
        // from a single enqueue aborted the whole provider run and silently took
        // the remaining cards of the page with it. A card that cannot be recorded
        // is a card lost either way, but losing one is not the same as losing the
        // rest of the run, and the count is published so it stops being invisible.
        try {
