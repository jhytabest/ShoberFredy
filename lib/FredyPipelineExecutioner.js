...
import { ProviderTransientError } from './services/pipeline/providerErrors.js';
...
class FredyPipelineExecutioner {
  constructor(providerConfig, job, providerId, browser) {
    ...
    this._jobKey = job.id;
    this._jobMaxPages =
      job.provider?.find((entry) => entry.id === providerId && entry.url === providerConfig.url)?.maxPages ?? null;
  ...
  async execute() {
    try {
      ...
      await this._providerConfig.releaseDiscovery?.(this._browser);
    }
  }
  ...
  async _discoverPages(baseUrl) {
    ...
    for (let page = 1; page <= maxPages; page++) {
      ...
      try {
        const discovered = await (this._providerConfig.getListings?.(url, this._browser) ?? this._getListings(url));
        ...
        logger.warn(
          `Discovery for '${this._providerId}' failed on page ${page}; earlier cards were already queued.`,
          error,
        );
        throw error;
      }
      if (raw.length === 0) {
        if (page === 1 && !containerPresent) {
          throw new ProviderTransientError(`Search results container is missing for '${this._providerId}'`);
        }
        if (page === 1) logger.debug(`Search for '${this._providerId}' returned no listings.`);
        break;
      }
  ...
  // Swallowing this returned undefined to the scheduler, which read it as a run
  // that found nothing and recorded the signal `empty`. A portal answering 403 and
  // a search that legitimately matched no flats are opposite facts, and the
  // breaker was being handed the second one for both: `Pausing 'immowelt' for
  // 41 min: discovery empty` was in fact eighty-six bot challenges. Provider
  // errors carry their own kind, so they are re-thrown for the caller to classify.
  // A geocoding outage still aborts the run quietly, because it is our failure
  // rather than the portal's and says nothing about whether the portal is healthy.
  _handleError(err) {
