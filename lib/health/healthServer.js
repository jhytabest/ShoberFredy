...
import { providerDiscoveryHealth } from '../services/jobs/providerCircuitBreaker.js';
...
export async function startHealthServer(port) {
  ...
  fastify.get('/health', async (_request, reply) => {
    ...
    const awaitingProxy = new Set(providersAwaitingProxy());
    ...
    return reply.code(healthy ? 200 : 503).send({
      ...
      providers: providerDiscoveryHealth().map((provider) => ({
        ...provider,
        awaitingProxy: awaitingProxy.has(provider.provider),
      })),
