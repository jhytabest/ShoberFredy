...
export const send = ({ serviceName, newListings = [], adapter, jobKey, signal, deliveredChatIds = [], onReceipt }) => {
  if (!adapter?.fields) {
  ...
  if (!chatIds.length) throw new Error('Telegram chatId contains no recipients');
  ...
  const allPromises = chatIds
    .filter((id) => !deliveredChatIds.includes(id))
    .flatMap((id) => {
      ...
      return newListings.map(async (listing) => {
        const response = await sendListingToChat(throttledCall, listing, id, opts);
        await onReceipt?.(id, response);
        return response;
      });
    });
