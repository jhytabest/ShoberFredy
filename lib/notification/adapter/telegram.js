 ...
 * @returns {string}
 */
function buildBody(jobName, serviceName, o, baseUrl, html) {
  ...
  if (baseUrl && o.id) {
    const fredy = `${baseUrl}/#/listings/listing/${o.id}`;
    lines.push(html ? `<a href='${esc(fredy)}'>Open in Fredy</a>` : `Open in Fredy: ${fredy}`);
  }
  return lines.join('\n');
}

function buildHtmlBody(jobName, serviceName, o, baseUrl) {
  return buildBody(jobName, serviceName, o, baseUrl, true);
}

// Telegram photo captions are capped at 1024 characters; the fallback
// sendMessage path (uncapped) carries the full body when the caption overflows.
function buildPlainCaption(jobName, serviceName, o, baseUrl) {
  return buildBody(jobName, serviceName, o, baseUrl, false).slice(0, 1024);
}

function buildPlainText(jobName, serviceName, o, baseUrl) {
  return buildBody(jobName, serviceName, o, baseUrl, false);
}
 ...
 */
export const send = ({ serviceName, newListings = [], adapter, jobKey, baseUrl }) => {
  if (!adapter?.fields) {
