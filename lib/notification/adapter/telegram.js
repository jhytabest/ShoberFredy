...
/**
 * Render the curated facts line, falling back to bare price and size.
 *
...
/**
 * Build the Telegram message body in HTML or plain mode. Renders the curated
 * facts, address, market-score line and the LLM summary that the dispatcher
...
function buildBody(jobName, serviceName, o, html) {
  ...
  if (o.scoreLine) lines.push(esc(o.scoreLine));
  if (o.summary) lines.push(html ? `<i>${esc(o.summary)}</i>` : o.summary);
  // Last, so that the caption clipper drops it before anything else: the
  // structured facts and the summary are what a reader scans, and the comments
  // are the long tail the schema no longer has fields for.
...
/**
 * Clip an HTML caption to Telegram's limit without splitting markup.
 *
 * A plain `.slice(0, CAPTION_LIMIT)` cut through whichever `<a href='…'>` or
 * `<i>` happened to straddle the boundary, and Telegram rejected the entire
 * caption — "Can't find end tag corresponding to start tag \"a\"" — which cost
 * the photo and two retries before the uncapped sendMessage fallback carried
 * the text. Every line {@link buildBody} emits is independently balanced, so
 * dropping whole trailing lines is the one truncation that cannot leave invalid
 * markup behind.
 *
 * @param {string} html
 * @returns {string}
...
function clipHtmlCaption(html) {
  ...
  // Unreachable for a real listing — the first line is the job name — but a
  // single oversized line can only be shortened by removing the markup it
  // straddles. Trailing partial entities go too, since `&am` is also a parse
  // error.
  return html
    .replace(/<[^>]*>/g, '')
...
// Plain text carries no markup, so a hard cut is safe here. The uncapped
// sendMessage fallback still carries the full body when the caption overflows.
 ...
 * @param {Object} params.adapter - This job's Telegram configuration entry.
