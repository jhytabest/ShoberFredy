...
/**
 * Build a Telegram photo upload directly from Fredy's durable media store.
 * This avoids Telegram fetching an expired or bot-protected portal URL.
 */
export async function buildLocalPhotoFormData({ chatId, imagePath, caption, parseMode, messageThreadId }) {
  ...
  if (parseMode) fd.append('parse_mode', parseMode);
  if (messageThreadId != null) fd.append('message_thread_id', String(messageThreadId));
  // Stored files are optimized WEBP; convert them to a real JPEG so Telegram
  // never has to infer the media type from mismatched bytes and filename.
  fd.append('photo', blob, 'photo.jpg');
  return fd;
}
