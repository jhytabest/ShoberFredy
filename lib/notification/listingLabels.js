...
function availabilityLabelDe(row) {
  ...
  if (row.availability === 'date' && row.availableFrom) return `ab ${row.availableFrom}`;
...
function furnishingLabelDe(row) {
  ...
  if (row.furnishingStatus === 'full' || row.furnished === true) return 'möbliert';
...
function petsLabelDe(row) {
  ...
  if (row.petsPolicy === 'prohibited' || row.petsAllowed === false) return 'keine Haustiere';
  if (row.petsPolicy === 'allowed' || row.petsAllowed === true) return 'Haustiere erlaubt';
  return null;
}
