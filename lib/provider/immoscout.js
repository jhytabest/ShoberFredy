...
async function fetchExpose(listing) {
  ...
  return await detailed.json();
}

async function captureDetails(listing) {
  const detailBody = await fetchExpose(listing);
  if (!detailBody) throw new Error(`ImmoScout expose capture failed for ${listing.link}`);
  const exposeId = listing.link?.split('/').pop();
...
const config = {
  ...
  pagination: { urlForPage: (url, page) => queryPage(url, page, 'pagenumber'), maxPages: 20 },
  captureDetails,
