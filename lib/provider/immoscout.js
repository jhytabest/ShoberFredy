...
function exposeHttpError(response, exposeId) {
  const message = `ImmoScout expose ${exposeId} returned ${response.status} ${response.statusText}`;
  const options = { status: response.status };
  if (response.status === 404 || response.status === 410) return new ProviderInactiveError(message, options);
  ...
  if (response.status >= 500 || [408, 425].includes(response.status)) {
    return new ProviderTransientError(message, options);
  }
  return new ProviderPermanentError(message, options);
}
...
async function isListingActive(link) {
  try {
    ...
    if ([401, 403, 429].includes(result.status)) return -1;
    return 0;
  } catch {
    return 0;
  }
