...
async function getListings(url, _browser, signal) {
...
async function fetchExpose(listing, parentSignal) {
  ...
  try {
    return await withOperationDeadline(
      ...
      { timeoutMs: 60_000, name: label, signal: parentSignal },
    );
  } catch (error) {
...
async function captureDetails(listing, _browser, signal) {
  const detailBody = await fetchExpose(listing, signal);
  const exposeId = listing.link?.split('/').pop();
