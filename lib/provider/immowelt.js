...
const config = {
  ...
  puppeteerOptions: {
    ...
    // The listing API round-trip lands a second or two after domcontentloaded.
    // The old 60s ceiling was never reached on a healthy page — it was only
    // ever spent in full on challenge pages, which never go idle at all.
...
export const metaInformation = {
  ...
  id: 'immowelt',
  // Immowelt answers this host's datacenter IP with an HTTP 403 bot challenge
  // and no cards, so running it without a proxy only feeds the circuit breaker
  // until it sits at the six-hour ceiling. Discovery and detail capture both
  // skip the provider while 'proxyUrl' is empty.
  requiresProxy: true,
};
export { config };
