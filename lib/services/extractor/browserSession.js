...
// Discovery and detail capture both share one browser behind one lock,
...
// never reached. Every path below now releases, waits are bounded, and the
// ceiling kills the browser rather than waiting politely, because a dead browser
// is what makes a wedged CDP call reject and lets an operation nobody can cancel
// unwind. Reaching the lock timeout means the ceiling did not do its job, so it
// surfaces as a transient provider error and the breaker takes it from there.
