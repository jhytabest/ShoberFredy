...
} from '../pipeline/providerErrors.js';
...
export async function launchBrowser(url, options) {
  ...
  return await launch({
    ...
    // A ceiling on any single CDP call. Without it the default leaves a wedged
    // renderer able to hold a call open for minutes, past the caller's own
    // deadline — the navigation and selector timeouts below only cover the calls
    // that take a timeout in the first place.
    launchOptions: { protocolTimeout: env('FREDY_BROWSER_PROTOCOL_TIMEOUT_MS') },
    ...(preCfg.timezone ? { timezone: preCfg.timezone } : {}),
  });
}

// The graceful path, for a browser that is still answering.
export async function closeBrowser(browser) {
  if (!browser) return;
  try {
    await withDeadline(browser.close(), env('FREDY_BROWSER_CLOSE_TIMEOUT_MS'));
  } catch {
    // close() talks the same protocol as the work that just failed, so when it
    // is the protocol that is stuck, close() hangs with it. Escalate.
    await destroyBrowser(browser);
  }
}

// The ungraceful path: kill the process. Every pending CDP call rejects as the
// pipe closes, which is what lets an operation nobody can cancel unwind. tini is
// PID 1 in the container (`init: true`), so the renderer children are reaped.
export async function destroyBrowser(browser) {
  if (!browser) return;
  try {
    ...
    // No handle to the process — a remote or wrapped browser. Best effort.
    await withDeadline(browser.close(), env('FREDY_BROWSER_CLOSE_TIMEOUT_MS'));
  } catch {
    // ignore: there is nothing further to try, and the caller is already failing
  }
}
...
export default async function execute(url, waitForSelector, options) {
  ...
  let result;
  let failure = null;
  // A caller that reaches here without a URL used to spend a browser launch and
  // a navigation before CDP rejected it, and the resulting "Failed to
  // deserialize params.url" surfaced as a retryable transient fault — so the
  // item retried, deferred and finally died without ever naming the real
  // problem. No amount of retrying invents a link, so refuse it up front.
  if (typeof url !== 'string' || url.trim() === '') {
    throw new ProviderPermanentError(`Browser request needs a URL, got ${JSON.stringify(url)}`);
  }
  try {
    ...
    if (challengedByStatus || botDetected(botProbe ?? pageSource, statusCode)) {
      logger.warn(`Provider returned a bot-detection challenge. Url: ${url}`);
      ...
      const listings = await page.evaluate(extractDiscoveryListings, discoverySchema);
      // A results container holding no cards is an empty search; no container at
      // all means we were served something that is not a results page. Both
      // yield zero listings but mean opposite things, so establish which while
      // the page is still open.
      const containerPresent = listings.length
        ? true
        : await page.evaluate(
            (selector) => Boolean(document.querySelector(selector)),
            discoverySchema.crawlContainer.split(' ')[0],
          );
      result = { listings, containerPresent };
...
function extractDetailListing({ rootSelectors, embeddedSelectors, gallerySelectors, scanHtmlImages }) {
  ...
  const parseJsonScript = (raw) => {
    ...
    try {
      ...
      // Continue with the JSON.parse('...') wrapper below.
    }
    const call = value.match(/JSON\.parse\(\s*(["'])/);
    if (!call) return null;
    const quote = call[1];
