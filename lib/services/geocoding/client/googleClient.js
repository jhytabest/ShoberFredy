...
/*
 * Google Geocoding client used by the ingestion pipeline
...
/**
 * Canonical form of a street name for comparison.
 *
 * Portals write German compounds run together and abbreviated
 * ("Eberswalderstr. 34", "Greifswalderstraße 220") while Google returns the
 * spaced official name ("Eberswalder Straße"). Comparing those with a
 * substring test failed on the space, the street candidate was rejected as
 * implausible, and the address fell back to a postcode centroid. Folding the
 * suffix and removing every separator makes the two spellings identical.
 */
function normalizeRoute(value) {
  return (
    String(value || '')
      .toLowerCase()
      .replace(/ß/gu, 'ss')
      // House numbers ("108a") carry no street identity.
      .replace(/\d+[a-z]?/gu, '')
      // "str" / "str." -> "strasse"; the boundary stops it matching inside
      // "strasse" itself, so the substitution never doubles up.
