...
function pauseUntil() {
  return (
    Number(
      getCacheDb().prepare("SELECT value FROM pipeline_control WHERE name = 'geocode_paused_until'").get()?.value,
    ) || 0
  );
}

function pauseGeocoder() {
  const until = Date.now() + PAUSE_ON_ERROR_MS;
  getCacheDb()
    .prepare(
      `INSERT INTO pipeline_control(name, value, updated_at) VALUES ('geocode_paused_until', ?, ?)
    ON CONFLICT(name) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`,
    )
    .run(String(until), Date.now());
  return until;
}
...
export async function geocodeAddress(address, { city = null, signal, audit = {} } = {}) {
  ...
  if (cached) {
    logger.event('geocode_cache_hit', 'debug', {
      ...audit,
      addressKey: key,
  ...
  if (!isGoogleGeocodingConfigured()) {
    warnMissingKeyOnce();
    logger.event('geocode_deferred', 'debug', { ...audit, addressKey: key, pausedUntil: pauseUntil() });
  ...
  try {
    const result = await googleGeocodeAddress(address, { city, signal, audit });
    if (result) {
      writeCache({
        addressKey: key,
