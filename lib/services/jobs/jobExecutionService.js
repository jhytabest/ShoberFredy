...
import { recordDiscoveryRun, isProviderPaused, pausedForMs, recordProviderSignal } from './providerCircuitBreaker.js';
...
export function initJobExecutionService({ providers }) {
  ...
  async function executeJobProvider(job, prov, matchedProvider) {
    ...
    const execute = (browser, signal) =>
      new FredyPipelineExecutioner(runtimeConfig, job, prov.id, browser, { maxPages: prov.maxPages, signal }).execute();
    ...
    try {
      ...
      const discovered = await withOperationDeadline(
        (signal) =>
          usesBrowser
            ? withBrowserSession(runtimeConfig.url, { signal }, (browser) => execute(browser, signal))
            : execute(null, signal),
