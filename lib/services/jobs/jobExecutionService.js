    ...
    try {
      ...
      for (const prov of jobProviders) {
        try {
          ...
          // The blacklist is visibility-only and owned by the pipeline
          // (FredyPipelineExecutioner._evaluateVisibility); providers get an
          // empty list so their filter functions only drop broken rows.
