...
// longer than the scheduling interval itself.
const DISCOVERY_TIMEOUT_MS = positiveEnv('FREDY_DISCOVERY_TIMEOUT_MS', 2 * 60 * 1000);
      ...
      for (const prov of jobProviders) {
        ...
        try {
          ...
          try {
            ...
            // An empty result is the same observable outcome whether the cause
            // is a challenge, a proxy outage, or changed markup — and in all
            // three, repeating it for the next job this run cannot help.
            if (Array.isArray(discovered) && discovered.length > 0) recordProviderSuccess(prov.id);
            else recordProviderFailure(prov.id, 'no cards discovered');
          } catch (err) {
            ...
            recordProviderFailure(prov.id, err instanceof OperationDeadlineError ? 'deadline' : 'error');
            throw err;
          }
        } catch (err) {
          logger.error(err);
