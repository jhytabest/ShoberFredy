...
import { recordDiscoveryRun, isProviderPaused, pausedForMs, recordProviderSignal } from './providerHealth.js';
  ...
  async function executeJobProvider(job, prov, matchedProvider) {
    ...
    try {
      ...
      const discovered = await withOperationDeadline(
        (signal) => (usesBrowser ? withBrowserSession(runtimeConfig.url, { signal }, execute) : execute()),
