...
export function recordProviderSignal({ provider, market, scope, signal }, now = Date.now()) {
  ...
  if (signal === 'ok' || (scope === 'discovery' && signal === 'empty')) {
    ...
    // captures report scope 'item', and one of those
...
// that is refusing us; when the reason every portal refused us was our own
// egress going away, holding a 30-minute pause against them means the cooldown
// outlives every window in which it could be tested.
...
export function recordDiscoveryRun(job, source, outcome, count = null, now = Date.now()) {
  db()
    .prepare(
      `INSERT INTO discovery_run_audit
    (job_id, provider, source_url, outcome, listing_count, finished_at)
    VALUES (?, ?, ?, ?, ?, ?)`,
    )
    .run(job.id, source.id, source.url, outcome, count, now);
}
