...
export function claimsForListing(listing) {
  ...
  // Not namespaced by job. The capture hash already covers the provider and the
  // source key and never depended on which search found the advert — the job
  // prefix was the single reason two jobs finding one advert produced two
  // listings, because each one's replay claim was invisible to the other.
  ...
  // A hash of the bytes we re-encoded, so this is byte identity: it catches the
  // same CDN image reached twice, not the same photograph published by two
  // portals, which each serve their own encoding. Cross-portal identity is `geo`
  // below; treating `img` as if it could do that job is why it never once fired
  // across two portals.
  ...
  // building, the same flat listed twice is two points in one place with one
  // floor area. Titles get rewritten between portals and galleries get
  // re-encoded, but the building does not move — so this is the only kind that
  // can identify one advert across two portals, and it is the reason the
  // cross-portal count has been exactly zero.
  //
  // The rent used to be in this key alongside the area, and that is what kept it
  // at zero: one portal quotes the Kaltmiete and the next the Warmmiete for the
  // same flat, twenty to thirty per cent apart, so the two never landed in one
  // 3% bucket and the index never offered the candidate. Nothing downstream ever
  // got the chance to judge the pair.
  //
  // Area stays, so the key is still not bare coordinates: the whole building
  // would be one claim and the first flat in it would shadow every other. The
  // rent moves to the veto, where two figures can be compared knowing which
  // basis each is on — which an index key cannot do.
  ...
  if (size != null && lat != null && lng != null && TRUSTED_ACCURACIES.has(accuracy)) {
    for (const latCell of cellBuckets(lat)) {
      for (const lngCell of cellBuckets(lng)) {
        ...
        // half a square metre would have been narrower than the tolerance it is
        // supposed to feed.
 ...
 *
 * A match may be owned by a listing or by a rejected source — exactly one of the
 * two, enforced by the table. Callers that only care about listings filter on
 * `listing_id`; the terminal-verdict gate is the one that wants both, because
 * "we already refused this advert without ever extracting it" is the cheapest
 * answer there is and it has no listing behind it.
 *
 * @param {import('better-sqlite3').Database} db
 * @param {({claim: string}|string)[]} claims
 * @param {{excludeListingId?: string|null}} [options]
 * @returns {{listing_id: string|null, source_id: string|null, kind: string, claim: string}[]}
 */
...
export function recordClaims(db, listingId, claims, now = Date.now()) {
  ...
  // First writer wins, with one exception: a claim currently owned by a rejected
  // source is promoted when a listing asserts it. That is not a claim changing
  // hands, it is the same advert being extracted at last — a rejection is a
  // placeholder for facts nobody has bought yet, and a listing outranks it. The
  // guard keeps the asymmetry: a source never takes a claim from a listing.
...
/**
 * Record the claims a rejected source asserts.
 *
 * A rejection needs to be recognisable for exactly the same reason a listing
 * does: an advert nobody can identify is an advert that gets fetched, extracted
 * and refused again on the next capture whose page text differs at all. It does
 * not need a `listings` row to hold that memory — it needs a claim.
 *
 * @param {import('better-sqlite3').Database} db
 * @param {string} sourceId
 * @param {({claim: string, kind: string})[]} claims
 * @param {number} [now]
 * @returns {number} claims newly owned by this source
 */
export function recordSourceClaims(db, sourceId, claims, now = Date.now()) {
  ...
  // Never steals: a claim a listing owns stays with the listing.
  const insert = db.prepare(
...
export function vetoes(left, right) {
  ...
  if (kind === 'geo') {
    ...
    // Two rents only contradict each other when they measure the same thing. A
    // Kaltmiete of 900 and a Warmmiete of 1,150 for one flat are not a conflict,
    // they are the Nebenkosten; comparing them anyway is what made this kind
    // useless between portals.
    if (comparableRents(left, right)) {
      ...
      // With the rent out of play the room count has to carry it, and it has to
      // be stated on both sides rather than fail open: two flats in one building
      // with floor areas within 5% are common, and without either the rent or the
      // room count there is nothing left that says they are one flat.
 ...
 * `kinds` restricts which claim kinds may match. Passing the identity set makes
 * this pure dedupe: identity kinds are never vetoed and never consult a verdict,
 * so the call can be made before any filter has run.
 *
 * @param {import('better-sqlite3').Database} db
 * @param {object} listing canonical listing or stored row
 * @param {{claims?: {claim: string, kind: string}[], excludeListingId?: string|null,
 *          kinds?: Set<string>, acceptedAnywhere?: boolean}} [options]
...
export function resolveListingMatch(
  ...
  //
  // The flag is `accepted`, not `hidden`, and the direction is the point. When it
  // was `hidden`, a row read with `SELECT *` that had lost the column produced
  // `undefined`, `Boolean(undefined)` was false, every stored listing looked
  // visible, and a resemblance guess could silence one — silently, with no error
  // anywhere. Asking whether a row is *accepted* makes a missing answer mean "not
  // accepted", so the same mistake costs a missed merge and a duplicate message
  // instead. That is the cheap direction, and the file already says so above.
...
/**
 * Load the candidate rows, with the one derived fact resolution needs.
 *
 * `accepted` is selected explicitly rather than read off the row, because it is
 * not a column on `listings` any more — a verdict belongs to a job, and this
 * asks whether *any* job wants the listing. Selecting it here keeps the whole
 * question in one query instead of a per-row lookup inside the comparator.
 *
 * `price_basis` comes along for the same reason: `listings.price` is a Kaltmiete
 * for some rows and a Warmmiete for others, and a veto that compares two rents
 * has to know which is which. Without it, one portal's cold rent and another's
 * warm rent for the same flat read as a price conflict.
 */
...
/** Which rent `price` is: 'cold', 'warm', or nothing positively known. */
...
/** Whether the two sides quote the same kind of rent, so the figures can be compared. */
