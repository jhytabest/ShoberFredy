 ...
 *    jobs and providers (upstream's cross-provider dedupe).
 ...
 * @param {{similarityCache: {checkAndAddEntry: Function}, providerId: string}} options
...
export function dropDuplicates(listings, { similarityCache, providerId }) {
  ...
  return listings.filter((listing) => {
    try {
      const similar = similarityCache.checkAndAddEntry({
        title: listing.title,
      ...
      const match = db ? findStoredDuplicate(db, listing) : null;
      if (match) {
        logger.info(
