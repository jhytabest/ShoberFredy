 ...
 *
 * Includes visible and filtered production listings and preserves every
 * absorbed row as a compressed audit bundle. Exact sources, provider IDs,
...
import { addressTokenKey, addressesCompatible } from '../listings/dedupe.js';
import { collectListingAuditBundle, compressAuditBundle } from '../pipeline/listingAuditBundle.js';
...
const GEO_CELL_DECIMALS = 3; // ~111 m bucketing, so pairs are only compared locally
...
function connectTrustedGeo(db, listings, connect) {
  ...
  for (const listing of listings) {
    ...
    // Bucket by a coarse cell rather than by exact size: sizes that differ by a
    // few per cent are the same flat and must still land together.
    const key = `${listing.user_id}|${latitude.toFixed(GEO_CELL_DECIMALS)}|${longitude.toFixed(GEO_CELL_DECIMALS)}`;
