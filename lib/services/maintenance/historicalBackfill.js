...
/*
 * One-time repair of rows written before the current rules existed.
 *
 * ------------------------------------------------------------------------------
 * THIS FILE IS MEANT TO BE DELETED. To retire it: delete this file, remove the
 * import and the single `runHistoricalBackfill(db)` call in maintenanceWorker.js,
 * and drop the `backfill_*` rows from `pipeline_control`. Nothing else refers to
 * it, and no other module imports anything from here.
 * ------------------------------------------------------------------------------
 *
 * It does not live in the migration. `100.current-schema.js` had accumulated six
 * one-time backfills, which turned the file that defines the current schema into
 * a log of every past one — and because the runner permits exactly one migration
 * file, nothing there can ever be retired. Upkeep that runs once belongs
 * somewhere that can be thrown away.
 *
 * Every pass is guarded by its own marker, so a crash halfway leaves the finished
 * ones finished, and re-running is a no-op rather than a second application.
 *
 * The hard constraint: no pass may cost an LLM call or a geocoding call. Every
 * one of them reads only text, attributes and the geocode cache that are already
 * stored, which is what makes repairing 2,142 listings free.
 */
...
/**
 * A Warmmiete reads as one of these and never as a Kaltmiete. Deliberately
 * narrow: the point is to be sure, not to classify everything.
 */
...
/**
 * Run every unfinished pass. Called from scheduled maintenance, which already
 * holds a lease and a timeout.
 *
 * @param {import('better-sqlite3').Database} db
 * @returns {Record<string, number>} what each pass touched, for the audit payload
 */
export function runHistoricalBackfill(db) {
  const summary = {};
  // Order matters in one place only: the rent repair must precede the scoring
  // enqueue, or 257 warm rents get scored against a Kaltmiete model.
  for (const [marker, pass] of [
    ...
    try {
      ...
      // One failing pass must not block the others, and must not be marked done.
...
/**
 * A stated Warmmiete filed as a Kaltmiete.
 *
 * The instruction the model used to get said "state both, subtracting the
 * Nebenkosten from a stated Warmmiete", and when an advert gave only one figure
 * it obliged by writing that figure into both fields. 262 listings ended up with
 * cold exactly equal to warm; the stored page text says warm for 257 of them and
 * cold for none. Those cold rents are fiction, they are in the training corpus as
 * cold observations, and they pass a maxPrice filter a real Kaltmiete would fail.
 *
 * The text decides, so this costs nothing. Where the text is genuinely ambiguous
 * the figure becomes warm as well: warm >= cold, so calling a true Kaltmiete warm
 * only withholds it from training, while the reverse inflates the target.
 */
function repairConflatedRents(db) {
  ...
  db.transaction(() => {
    for (const row of rows) {
      ...
      if (saysColdOnly) continue; // the figure really is a Kaltmiete; drop the warm instead
      llm.rent.cold = null;
      ...
      if (!seen.has(row.listing_id)) {
        ...
        // `listings.price` keeps the same number: it was always this figure, and
        // the figure was always the Warmmiete. Only its name was wrong.
        repaired += 1;
      }
    }
  })();
...
/**
 * Listings restored from the pre-migration snapshot have no `listing_sources`
 * row, because the recovery script rebuilt listings, texts, verdicts and claims
 * without them. One shape means giving them one: everything a source needs is
 * derivable — the job from the verdict that already exists, the provider and URL
 * from the listing, the key from the URL.
 */
function synthesiseLegacySources(db) {
  ...
  db.transaction(() => {
    for (const row of rows) {
      ...
      // No detail_queue_id and no parsing_queue_id: no work item ever existed for
      // these, and inventing one would make the queue claim work with no payload.
...
/**
 * The `geo` claim key changed shape — the rent left it, so one portal's
 * Kaltmiete and another's Warmmiete stop landing in different buckets — but
 * claims are first-writer-wins and are never rewritten. 11,068 keys are still in
 * the old `lat|lng|price|size` shape against 160 in the new one, and an old key
 * can never collide with a new one, so the fix was inert on everything already
 * stored.
 *
 * Re-minting reads `homeserver_geocode_cache` and nothing else, so no address is
 * geocoded again. Only `geo` is touched: the identity kinds are proofs and must
 * keep their history.
 */
...
/**
 * 1,328 verdicts predate the per-job configuration hash, so every measurement of
 * the funnel silently excluded them and the dedupe gate could not tell whether
 * they still bound. The filters are pure functions of stored attributes, so
 * re-deciding them is free.
 *
 * Nothing may be announced as a result. A re-decided verdict can flip a
 * two-month-old rejection into an acceptance, and 1,300 old flats arriving on
 * Telegram would be the worst possible outcome of a tidy-up, so every re-decided
 * acceptance is stamped as already notified.
 */
  ...
  db.transaction(() => {
    for (const row of rows) {
      ...
      recordVerdict(db, {
        ...
        reason,
        stage: primaryFilterStage(reasons) ?? 'final',
...
/**
 * The migration cleared every stored score, correctly — they were computed from a
 * price on a basis that no longer exists — but nothing re-rated the backlog, so
 * coverage sat at 2 listings out of 1,580 eligible and every model panel on the
 * dashboard read empty. Rating is local: the artifacts and the geocode cache,
 * no model call and no geocoder.
 */
function enqueueScoreBacklog(db) {
  ...
  for (const row of rows) {
    ...
    // notify: false — this is a rescore of adverts the user has already seen.
...
/**
 * Nine accepted verdicts carry no `notified_at` and have no notification work
 * left to deliver them, all from before the keying fix landed. Nothing will pick
 * them up on its own.
 */
