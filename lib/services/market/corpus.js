 ...
 * different data.
 *
 * Target policy: the Kaltmiete, and nothing else.
 *
 * A stated cold rent, or the stored price when the listing's own basis says it
 * is cold. Anything else is not trainable — mixing warm prices into a cold €/m²
 * target biases every neighbourhood where they concentrate.
 *
 * There used to be a third source: warm minus the parsed Nebenkosten and
 * Heizkosten, labelled 'cold_est'. It reached two listings out of 2,098, because
 * adverts that quote a Warmmiete rarely itemise the charges — and it cost a
 * derivation here, a guard against yearly-instead-of-monthly charges, a feature
 * flag in both models to mark the estimate, two extraction fields and a
 * consistency rule over them. An advert that states only a warm rent is simply
 * not a cold observation, and saying so is cheaper than inventing one.
 *
...
/**
 * The Kaltmiete of a listing, or null when it does not state one. Shared
 * verbatim between corpus construction (training) and the notification-time
 * scorer, so training target and scored quantity can never diverge.
 *
 ...
 * @returns {{
 *   rows: object[],          // every unique flat, cold rent or not
 *   trainingRows: object[],  // the ones with a Kaltmiete, outliers trimmed
  ...
  return db
    .prepare(
      ...
      SELECT
        ...
        l.address, l.link, l.provider, l.latitude, l.longitude, l.state,
      ...
      WHERE EXISTS (
          ...
          WHERE v.listing_id = l.id
            ...
            AND (v.reason IS NULL OR v.reason NOT IN ('blacklist', 'no_detail'))
        )
...
/**
 * Read the canonical attribute document selected above.
 */
...
function enrichRow(row, now, projection) {
  ...
  return {
    ...
    targetRent,
    size,
    // From the row, like `size`: the attributes carried an identical copy.
    ...
    // The certificate is disclosed by law and prices the flat. Only the class
    // is carried: `energyValueKwh` reached this shape too and no model term ever
    // read it, so it was extracted, stored and threaded through for nothing.
