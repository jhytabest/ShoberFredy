...
import { PIPELINE_SCHEMA_VERSION } from '../pipeline/pipelineVersion.js';
import { coordQualityTier } from './conformal.js';
import { buildProjection, gridKey } from './geo.js';
...
export function loadCorpus(db, now) {
  ...
  const referenceLatitude =
    median(unique.map((row) => Number(row.latitude)).filter((lat) => Number.isFinite(lat) && lat !== -1)) ??
...
function queryRows(db) {
  ...
  const cacheJoin = tableExists(db, 'homeserver_geocode_cache')
    ...
    : `LEFT JOIN (SELECT NULL AS address_key, NULL AS accuracy, NULL AS status WHERE 0) c
        ON c.address_key = homeserver_address_key(l.address)`;
  const hasAttributes = tableExists(db, 'listing_attributes');
  const hasAttributeSchema = hasAttributes && columnExists(db, 'listing_attributes', 'schema_version');
  const attrsJoin = hasAttributes ? `LEFT JOIN listing_attributes a ON a.listing_id = l.id` : '';
  const attrsFilter = hasAttributeSchema ? `AND a.schema_version >= ${PIPELINE_SCHEMA_VERSION}` : '';
  const canonicalFilter = columnExists(db, 'listings', 'canonical_schema_version')
    ? `AND l.canonical_schema_version >= ${PIPELINE_SCHEMA_VERSION}`
    : '';
  const optionalAttr = (column, alias) =>
    hasAttributes && columnExists(db, 'listing_attributes', column) ? `a.${column} AS ${alias}` : `NULL AS ${alias}`;
  const attrsSelect = hasAttributes
    ? `, a.parsed_at AS attrs_parsed_at, a.cold_rent_eur AS attr_cold_rent_eur,
       ...
       ${optionalAttr('bedrooms', 'attr_bedrooms')}, ${optionalAttr('bathrooms', 'attr_bathrooms')},
       ${optionalAttr('total_floors', 'attr_total_floors')},
       ${optionalAttr('furnishing_status', 'attr_furnishing_status')},
       ${optionalAttr('lease_type', 'attr_lease_type')},
       ${optionalAttr('amenities_json', 'attr_amenities_json')},
       ${optionalAttr('amenities_absent_json', 'attr_amenities_absent_json')},
       ${hasAttributeSchema ? 'a.schema_version' : PIPELINE_SCHEMA_VERSION} AS attr_schema_version`
    : '';
  // Blacklisted listings (WG rooms, furnished, temporary, ...) are priced on a
  // different market, and 'no_detail' legacy rows have no verified evidence —
  // keep both out. Spec/area-filtered listings ARE kept: they are valid Berlin
  // market observations (out-of-budget / out-of-polygon is a user preference,
  // not bad data) and give the spatial model its geographic spread.
  const blacklistFilter = columnExists(db, 'listings', 'hidden_reason')
...
function storedAttrs(row) {
  if (Number(row.attr_schema_version) < PIPELINE_SCHEMA_VERSION) {
    return { priceType: 'unknown', swap: false, features: structuredFeatureFlags() };
  }
  let features;
  try {
...
function collapseDuplicates(rawRows) {
  ...
  for (const row of byLink.values()) {
    if (!TRUSTED_ACCURACIES.has(row.geocode_quality) || !hasUsableCoords(row)) {
      unique.push(row);
      continue;
    }
...
function enrichRow(row, now, projection) {
  ...
  const usableCoords = hasUsableCoords(row);
  ...
  return {
    ...
    roomsBand: roomsBand(attrs.rooms),
...
/**
 * Size band label for reporting (not a model input).
 * @param {number} size
 * @returns {string}
 */
...
/**
 * Rooms band label for reporting (not a model input).
 * @param {number} rooms
 * @returns {string}
 */
export function roomsBand(rooms) {
