...
/*
 * Shoberfredy market model: geo-surface-v4.
 *
 ...
 * 2. A hierarchical residual field over the hedonic residuals: adaptive-
 *    bandwidth Gaussian kernel (bandwidth = distance to the k-th neighbour)
 *    with weighted medians, leave-one-out scoring, and empirical-Bayes
 *    segment priors by area/size/rooms bands for sparse neighborhoods.
 *
...
const MODEL_VERSION = 'geo-surface-v4';
...
const SEGMENT_PRIOR_MIN_WEIGHT = 4;
const SEGMENT_PRIOR_FULL_WEIGHT = 20;
...
function trainResidualField(listings, hedonic) {
  ...
  for (const observation of observations) {
    ...
    for (const segmentKey of segmentKeys(observation)) {
      if (!segments.has(segmentKey)) segments.set(segmentKey, []);
      segments.get(segmentKey).push(observation);
    }
  }
  return { index, segments };
}

function predictFieldAt(x, y, field, excludedListingId = null, listing = null) {
  ...
  const localResidualLog = weightedQuantile(points, 0.5);
  if (localResidualLog == null) return empty;

  const effectiveSamples = effectiveSampleSize(points);
  const prior = segmentPrior(field, listing, excludedListingId);
  const priorWeight =
    prior.effectiveSamples >= SEGMENT_PRIOR_MIN_WEIGHT
      ? Math.min(1, prior.effectiveSamples / SEGMENT_PRIOR_FULL_WEIGHT)
      : 0;
  const localWeight = Math.min(1, effectiveSamples / 12) * Math.sqrt(MIN_BANDWIDTH_M / bandwidthM);
  const residualLog =
    priorWeight > 0
      ? (localResidualLog * localWeight + prior.residualLog * priorWeight) / Math.max(localWeight + priorWeight, 1e-9)
      : localResidualLog;
  const spreadLog = Math.max(
    MIN_LOG_SPREAD,
    (weightedQuantile(points, 0.75) ?? 0) - (weightedQuantile(points, 0.25) ?? 0),
  );
  const confidence = clamp(Math.max(localWeight, priorWeight * 0.65), 0.05, 1);
  ...
  return {
    ...
    points,
    localResidualLog,
    segmentResidualLog: prior.residualLog,
    segmentEffectiveSamples: prior.effectiveSamples,
  };
}

function segmentPrior(field, listing, excludedListingId) {
  if (!listing || !field.segments) return { residualLog: 0, effectiveSamples: 0 };
  for (const key of segmentKeys(listing)) {
    const rows = (field.segments.get(key) || []).filter((row) => row.id !== excludedListingId);
    const points = rows.map((row) => ({ value: row.residualLog, weight: row.weight }));
    const residualLog = weightedQuantile(points, 0.5);
    if (residualLog != null && effectiveSampleSize(points) >= SEGMENT_PRIOR_MIN_WEIGHT) {
      return { residualLog, effectiveSamples: effectiveSampleSize(points), key };
    }
  }
  return { residualLog: 0, effectiveSamples: 0 };
}

function segmentKeys(listing) {
  return [
    `${listing.area}:${listing.sizeBand}:${listing.roomsBand}`,
    `${listing.area}:${listing.sizeBand}:*`,
    `${listing.area}:*:${listing.roomsBand}`,
    `${listing.area}:*:*`,
    `*:${listing.sizeBand}:${listing.roomsBand}`,
    `*:${listing.sizeBand}:*`,
    `*:*:${listing.roomsBand}`,
    `*:*:*`,
  ];
...
function scoreListing(listing, hedonic, field) {
  const surface = predictFieldAt(listing.x, listing.y, field, listing.id, listing);
  ...
  for (const key of cellKeys) {
    ...
    rows.push({
      ...
      surfaceComponentsJson: JSON.stringify({
        ...
        segmentResidualLog: roundMetric(surface.segmentResidualLog),
        segmentEffectiveSamples: roundMetric(surface.segmentEffectiveSamples),
      }),
    });
  }
  return rows;
...
function predictSplit(train, test) {
  ...
  return test.map((listing) => {
    const surface = predictFieldAt(listing.x, listing.y, field, null, listing);
