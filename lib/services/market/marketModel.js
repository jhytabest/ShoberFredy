...
const MIN_STARTUP_DELAY_MS = 60 * 1000;
...
/**
 * Delay the first scheduled retrain until the oldest live model is due.
 * Fresh artifacts survive container restarts, so retraining them again a
 * minute after every deploy only creates avoidable CPU and memory pressure.
 * A new or partially populated registry still receives its first run shortly
 * after startup.
 *
 * @param {number} [now]
 * @returns {number}
 */
export function marketModelInitialDelayMs(now = Date.now()) {
  ensureTables();
  const rows = db
    .prepare(`SELECT family, created_at FROM homeserver_models WHERE family IN (?, ?)`)
    .all(RIDGE_FAMILY, GBM_FAMILY);
  if (rows.length < 2) return MIN_STARTUP_DELAY_MS;

  const oldestArtifactAt = Math.min(...rows.map((row) => Number(row.created_at) || 0));
  const dueAt = oldestArtifactAt + config.intervalSeconds * 1000;
  return Math.max(MIN_STARTUP_DELAY_MS, dueAt - now);
}
