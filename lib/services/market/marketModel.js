...
export async function runMarketModelOnce() {
  ...
  const ridge = trainRidgeModel({ trainingRows, projection, now: startedAt, level });
  const gbm = await trainGbmModel({ trainingRows, now: startedAt, level });
  if (!ridge && !gbm) {
    logger.warn(
  ...
  persistRun({ startedAt, stats, results });

  const ridgeResult = results.find((result) => result.family === RIDGE_FAMILY);
  if (ridgeResult) {
    try {
