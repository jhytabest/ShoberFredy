...
import { corpusMarkets, loadCorpus } from './corpus.js';
...
export async function runMarketModelOnce() {
  ...
  const markets = corpusMarkets(db);
  if (!markets.length) {
    logger.warn('market model run skipped: no listing carries a market yet');
    return;
  }
  let requeue = false;
  for (const { market } of markets) {
    // One city failing to train is not the others' problem: each keeps or
    // replaces its own artifact.
    try {
      requeue = (await trainMarket(market)) || requeue;
    } catch (error) {
      logger.error(`market model run failed for '${market}'; its previous artifacts stay live`, error);
    }
  }
  if (requeue) {
...
async function trainMarket(market) {
  const startedAt = Date.now();
  const { rows, trainingRows, projection, stats } = loadCorpus(db, startedAt, market);
  ...
  if (!ridge && !gbm) {
    logger.warn(
      `market model run skipped for '${market}': corpus too small or trainers unavailable ` +
  ...
  const persistedResults = persistRun({ startedAt, market, stats, results });
...
function persistRun({ startedAt, market, stats, results }) {
  const insertRun = db.prepare(
    `INSERT INTO homeserver_model_runs
       (id, model_version, model_family, market, training_rows, scored_rows, created_at, metrics_json)
     VALUES (@id, @modelVersion, @modelFamily, @market, @trainingRows, @scoredRows, @createdAt, @metricsJson)`,
  );
  const persistedResults = [];
  for (const result of results) {
    try {
      db.transaction(() => {
        const runId = `${startedAt}-${market}-${result.family}`;
        insertRun.run({
          id: runId,
          modelVersion: result.version,
    ...
    .prepare(
      `SELECT r.*,
              (SELECT count(*) FROM homeserver_listing_model_scores s
               ...
               WHERE s.model_family = r.model_family
                 AND COALESCE(l.market, '') = COALESCE(r.market, '')) AS predictions
       ...
       WHERE r.created_at = (
         ...
         WHERE COALESCE(r2.model_family, '') = COALESCE(r.model_family, '')
           AND COALESCE(r2.market, '') = COALESCE(r.market, '')
       )
       ORDER BY r.market, r.model_family`,
    )
    .all();
}
