...
/*
 * A shipped feature-space change makes every stored artifact structurally
 * stale: both scorers reject on vector length and return null, so listings
 * complete unscored until a retrain lands. The key is the feature space rather
 * than a timestamp, so reboots are idempotent, a failed run stays visible and
 * retryable through the queue's own retry, and the next feature-space change
 * enqueues again on its own. It must not reuse the interval key, which would
 * dedupe away the next scheduled retrain.
 */
