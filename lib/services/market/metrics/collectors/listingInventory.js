...
// Trimmed to the series still plotted somewhere. Daily listings by provider
// and daily price per m2 moved to fredy-hunt as direct SQLite queries — a
// dashboard-only need doesn't justify a Prometheus series with a `date`
// label. Price quantiles, geocode quality, job freshness and the rest had no
// consumer either; /health and `yarn maintenance status` answer those on
// demand.
...
function enrichListing(row, cacheByAddress) {
  ...
  const cache = cacheByAddress.get(addressKey(address, row.market)) || null;
    ...
    .prepare(
      ...
      WITH versions AS (
        ...
        WHERE EXISTS (
            ...
            WHERE v.listing_id = l.id AND j.notify IS NOT NULL
          )
