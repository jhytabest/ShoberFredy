...
function emitPredictionMetrics(lines, db) {
  ...
  // so a confidence shown here is the confidence a listing was scored with.
  const ridgeArtifact = loadRidgeArtifact(db);
  const ridgeField = hydrateField(ridgeArtifact?.field);
  ...
  for (const { run, predictions } of snapshot) {
    emitFamilyPredictionMetrics(lines, run, predictions, { field: ridgeField, artifact: ridgeArtifact });
  }

  emitHistoryMetrics(lines, snapshot[0].predictions);
}

function loadRidgeArtifact(db) {
  if (!tableExists(db, 'homeserver_models')) return null;
  const row = db.prepare(`SELECT artifact_json FROM homeserver_models WHERE family = 'ridge'`).get();
