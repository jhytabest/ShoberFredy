...
/**
 * One scored listing joined to its listing row. The parsed room count lives in
 * listing_attributes, which older databases may not have, so the join is only
 * added when the column is really there.
 */
function buildScoreQuery(db) {
  const hasParsedRooms = tableExists(db, 'listing_attributes') && columnExists(db, 'listing_attributes', 'rooms');
  return db.prepare(
    `SELECT
       ...
       l.is_active, l.manually_deleted, l.address, l.latitude, l.longitude, l.title, l.link,
       l.provider, l.size AS size_sqm, l.created_at AS listing_created_at,
       ${hasParsedRooms ? 'COALESCE(a.rooms, l.rooms)' : 'l.rooms'} AS rooms
     FROM homeserver_listing_model_scores s
     JOIN listings l ON l.id = s.listing_id
     ${hasParsedRooms ? 'LEFT JOIN listing_attributes a ON a.listing_id = s.listing_id' : ''}
...
function emitFamilyPredictionMetrics(lines, latestRun, rows, localField) {
  ...
  const visibleRows = withUncertainty.filter((row) => row.manually_deleted !== 1);
  const activeRows = withUncertainty.filter((row) => row.is_active === 1 && row.manually_deleted !== 1);
