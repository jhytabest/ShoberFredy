...
function emitRejectionTerms(lines, db) {
  ...
  const inputs = [];
  if (sources) {
    inputs.push('SELECT reason, reason_term FROM source_rejections WHERE reason_term IS NOT NULL');
  }
  if (verdicts) {
    inputs.push(
      "SELECT reason, reason_term FROM listing_verdicts WHERE reason_term IS NOT NULL AND verdict = 'rejected'",
    );
  }
  const rows = db
    .prepare(
      `SELECT reason, reason_term, COUNT(*) AS n
       FROM (${inputs.join(' UNION ALL ')})
       GROUP BY 1, 2`,
    )
    .all();
  for (const row of rows) {
    metric(lines, 'fredy_rejections_by_term', row.n, {
      reason: row.reason ?? 'unknown',
      reason_term: row.reason_term,
    });
  }
}
