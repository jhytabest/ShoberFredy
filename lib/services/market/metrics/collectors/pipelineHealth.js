...
import { CLAIMABLE_SQL, CLAIMABLE_STATUSES } from '../../../pipeline/workQueue.js';
...
function notificationAgeSeconds(db) {
  ...
  const oldest = db
    .prepare(
      `SELECT min(created_at) AS created_at FROM pipeline_work
       WHERE kind = 'notify' AND status IN ${CLAIMABLE_SQL}`,
    )
    .get()?.created_at;
  return oldest ? Math.max(0, (Date.now() - oldest) / 1000) : 0;
}
...
function emitProviderMetrics(lines, runtimeSnapshot) {
  ...
  addHeader(lines, 'fredy_provider_consecutive_failures', 'gauge', 'Decayed weight of failed discovery runs.');
  addHeader(
    lines,
    'fredy_provider_challenge_score',
    'gauge',
    'Decayed weight of challenged single requests, which no longer pause a provider on their own.',
  );
  for (const provider of providers) {
    ...
    metric(lines, 'fredy_provider_challenge_score', provider.challenges ?? 0, labels);
  }
}
...
/**
 * How good the card filter actually is, per term.
 *
 * A card-stage refusal is never fetched, so on its own it is unfalsifiable. A few
 * percent are extracted anyway and graded against the sharper post-extraction
 * rule for the same thing (see `recordCardFilterAudit`), and this is where that
 * grade becomes readable. `card_wrong` is the series that matters: a term with a
 * rising share of it is refusing flats nobody asked it to refuse.
 *
 * Counted straight out of the audit ledger, which is permanent, so the ratio is
 * cumulative rather than a window — a term that fires twice a week needs months
 * before its share means anything.
 */
/**
 * Which rent each stored listing is on, and how many carry the one shape that
 * means the model failed to separate them.
 *
 * `conflated` is the series that matters. Under the previous extraction
 * instruction 40% of adverts returning both figures returned the same number
 * twice, and the stored page text said Warmmiete for 257 of them and Kaltmiete
 * for none — so a Warmmiete was being trained on and filtered against as if it
 * were cold. The instruction and a guard in the finalizer both close that now;
 * this is how a silent reopening becomes visible instead of taking two months to
 * notice.
 */
function emitRentBasisMetrics(lines, db) {
  if (!tableExists(db, 'listing_attributes')) return;
  addHeader(lines, 'fredy_listing_rent_basis', 'gauge', 'Stored listings by which rent their price is on.');
  for (const row of db
    .prepare(
      `SELECT COALESCE(json_extract(data, '$.priceType'), 'absent') basis, COUNT(*) n
       FROM listing_attributes GROUP BY 1`,
    )
    .all()) {
    metric(lines, 'fredy_listing_rent_basis', row.n, { basis: row.basis });
  }
  const conflated = db
    .prepare(
      `SELECT COUNT(*) n FROM listing_attributes
       WHERE json_extract(data, '$.coldRentEur') IS NOT NULL
         AND json_extract(data, '$.coldRentEur') = json_extract(data, '$.warmRentEur')`,
    )
    .get().n;
  metric(lines, 'fredy_listing_rent_basis', conflated, { basis: 'conflated' });
}
...
function emitCardFilterAuditMetrics(lines, db) {
  ...
  for (const row of rows) {
    metric(lines, 'fredy_card_filter_audit_total', row.count, {
      ...
      // agreed: the extraction refused it for the same reason.
      // other_reason: refused, but for something else.
      // card_wrong: the extraction found nothing wrong with it.
      verdict: row.action,
    });
  }
}
...
function emitQueueMetrics(lines, db) {
  if (tableExists(db, 'pipeline_work')) {
    ...
    // Why work ended, from the frozen vocabulary in workOutcome.js. This is the
    // series that makes the outcome policy legible: a code that suddenly
    // dominates is a policy bug, and until now the same question could only be
    // answered by reading free text out of last_error.
    const outcomes = db
      .prepare(
        `SELECT kind, outcome, COALESCE(outcome_code, '') AS code, COUNT(*) AS count
         FROM pipeline_work WHERE outcome IS NOT NULL GROUP BY kind, outcome, code`,
      )
      .all();
    addHeader(
      lines,
      'fredy_pipeline_work_outcomes',
      'gauge',
      'Terminal pipeline work items by kind, outcome and code.',
    );
    for (const row of outcomes) {
      metric(lines, 'fredy_pipeline_work_outcomes', row.count, {
        kind: row.kind,
        outcome: row.outcome,
    ...
    for (const kind of new Set(work.map((row) => row.kind))) {
      const oldest = work
        .filter((row) => row.kind === kind && CLAIMABLE_STATUSES.includes(row.status))
        .map((row) => row.oldest)
...
function emitNotificationMetrics(lines, db) {
  // In-flight is a lifecycle question and delivered is an outcome question, so
  // they are asked of different columns now rather than of one that meant both.
  const deliveries = db
    .prepare(
      `SELECT COALESCE(outcome, status) AS state, COUNT(*) AS count
       FROM pipeline_work WHERE kind = 'notify' GROUP BY state`,
    )
    .all();
  addHeader(lines, 'fredy_notification_deliveries', 'gauge', 'Notification outbox rows by state.');
  for (const state of ['pending', 'deferred', 'retry', 'sent', 'cancelled', 'failed']) {
    metric(lines, 'fredy_notification_deliveries', deliveries.find((row) => row.state === state)?.count ?? 0, {
      status: state,
    });
  }
  const notificationTiming = db
    .prepare(
      `SELECT
         MIN(CASE WHEN status IN ${CLAIMABLE_SQL} THEN created_at END) AS oldest_pending,
         MAX(CASE WHEN outcome = 'sent' THEN updated_at END) AS last_sent
       FROM pipeline_work
       WHERE kind = 'notify'`,
    )
    .get();
  ...
  const recentLatencies = db
    .prepare(
      `SELECT updated_at - created_at AS duration_ms
       FROM pipeline_work
       WHERE kind = 'notify' AND outcome = 'sent' AND created_at >= ?`,
    )
