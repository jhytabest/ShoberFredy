...
function emitServiceContractMetrics(lines, db, runtimeSnapshot) {
  ...
  check('live_pipeline_age', queueAgeSeconds(db, 'parse') <= LIVE_PIPELINE_SECONDS, 'warning');
  check('detail_fetch_queue_age', queueAgeSeconds(db, 'detail') <= DETAIL_QUEUE_SECONDS, 'warning');
