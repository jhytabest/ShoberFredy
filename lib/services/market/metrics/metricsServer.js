...
/**
 * Receive the main process's runtime state (geocoder health, worker
 * heartbeats). The exporter is a separate process and cannot observe them
 * directly, so they arrive over IPC and are published with their own age, which
 * lets a stale snapshot be told apart from a healthy one.
 *
