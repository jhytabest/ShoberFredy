...
export function localFieldLookup(db, surfaceRows) {
  ...
  for (const row of surfaceRows) {
    ...
    const [, cellX, cellY] = row.cell_id.split(':');
  ...
  return (latitude, longitude) => {
    if (!projection || !hasUsableCoordinates(latitude, longitude)) return null;
    const point = projection.project(Number(latitude), Number(longitude));
    return byCell.get(gridKey(point.x, point.y, SURFACE_CELL_M)) ?? null;
  };
}
