...
function emitPipelineMetrics(lines) {
  ...
  if (tableExists('listing_extractions')) {
    const modes = db
      .prepare(
        `SELECT COALESCE(parser_mode, 'pending') AS mode, COUNT(*) AS count FROM listing_extractions GROUP BY parser_mode`,
      )
      .all();
    addHeader(lines, 'fredy_listing_extractions', 'gauge', 'Listing extractions by parser mode.');
    for (const row of modes) metric(lines, 'fredy_listing_extractions', row.count, { mode: row.mode });
  }
  ...
  if (tableExists('notification_deliveries')) {
    ...
    for (const row of deliveries) metric(lines, 'fredy_notification_deliveries', row.count, { status: row.status });
  }
