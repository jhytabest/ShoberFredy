...
function emitModelArtifactHealth(lines) {
  if (!tableExists('homeserver_models')) return;
  const expectedLengths = {
    ridge: hedonicTermNames().length,
    gbm: gbmFeatureNames().length,
  };
  const models = db
    .prepare(`SELECT family, created_at, artifact_json FROM homeserver_models WHERE family IN ('ridge', 'gbm')`)
    .all();
  addHeader(
    lines,
  ...
  for (const family of ['ridge', 'gbm']) {
    ...
    const compatible = isArtifactCompatible(model?.artifact_json, family, expectedLengths[family]);
    metric(lines, 'fredy_market_model_artifact_compatible', compatible ? 1 : 0, { family });
  }
}

function isArtifactCompatible(artifactJson, family, expectedLength) {
  try {
    const artifact = JSON.parse(artifactJson || 'null');
    const actualLength = family === 'ridge' ? artifact?.beta?.length : artifact?.featureNames?.length;
