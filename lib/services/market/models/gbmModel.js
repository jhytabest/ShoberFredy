...
export const GBM_VERSION = 'gbm-quantile-v1';
export const GBM_FAMILY = 'gbm';
export const GBM_QUANTILES = { lo: 0.1, mid: 0.5, hi: 0.9 };
 ...
 * @param {object} [options.trainerOverrides] optional grid/round overrides
 *   forwarded to the Python trainer (tests use this to stay fast)
 ...
 */
export async function trainGbmModel({ trainingRows, now, level, pythonBin, trainerOverrides }) {
  if (trainingRows.length < MIN_TRAINING_ROWS) return null;
  const python = pythonBin || process.env.FREDY_PYTHON_BIN || 'python3';
...
/**
 * Batch score one corpus row for homeserver_listing_market_model. The GBM
 * has no local-comps notion, so z-score/percentile/comps columns are null —
 * its uncertainty story is the conformal interval.
 *
...
export function scoreGbmCorpusRow(row, prepared, artifact) {
  ...
  return {
    ...
    percentile: null,
    confidence: null,
    nearbyComps250m: null,
    nearbyComps500m: null,
    nearbyComps1000m: null,
    geoCell: row.geoCell,
    area: row.area,
