...
/*
 * Ridge market model, v5 ("ridge" family).
 *
 * Two-stage regression kriging on log cold-€/m²:
 * 1. a robust (Huber-IRLS to convergence) ridge regression on the
 *    STANDARDIZED hedonic design matrix — λ is selected by spatially-blocked
 *    cross-validation, not hardcoded;
...
export const RIDGE_VERSION = 'ridge-v8';
...
export function trainRidgeModel({ trainingRows, projection, now, level }) {
  ...
  const artifact = {
    ...
    surfaceCellSizeM: surfaceCellSizeFor(bandwidthClamps),
  ...
  const model = {
     ...
     *
     * There used to be an `atOwnTime` option choosing whether to predict at the
     * row's own month or at month zero. The month dummies it switched between
     * are gone, so the two answers had become the same one.
     * @param {object} row
     */
    predictRow(row) {
...
function fitHedonic(rows, { lambda }) {
  ...
  // Every observation counts the same. This was a recency-weighted quantile, and
  // an unweighted median is what that becomes once every weight is 1.
  beta[0] = median(y) ?? 0;
...
function surfaceCellSizeFor(clamps) {
  const half = (clamps.medianBandwidthM ?? 250) / 2;
  return Math.min(1000, Math.max(125, Math.round(half / 50) * 50));
}
...
function buildField(rows, hedonic, clamps) {
 ...
 * @returns {object}
 */
function fieldAt(x, y, field, excludedClusterId = null) {
  const candidates = [];
...
export function buildSurfaceCells(model, trainingRows, projection, minConfidence) {
  const cellSizeM = model.artifact.surfaceCellSizeM;
  const margin = Math.max(1, Math.round(250 / cellSizeM));
  const cellKeys = new Set();
  for (const row of trainingRows) {
    if (!row.hasCoords) continue;
    const cellX = Math.floor(row.x / cellSizeM);
    const cellY = Math.floor(row.y / cellSizeM);
