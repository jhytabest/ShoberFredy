...
import { detailDedupeKeys, preLlmBlacklistReason } from './temporaryDeterministic.js';
...
async function processDetail(detail, providerConfig, browser) {
  ...
  const job = getJob(detail.job_id);
  const hiddenReason = preLlmBlacklistReason(capture, detail.discovery, job?.blacklist || []);
  if (hiddenReason) {
    const listingId = markPreLlmHidden(detail, sourceHash, capture, hiddenReason);
    queueStorage.completeDetail(detail.id, null, capture);
    logger.info(`Soft-deleted pre-LLM blacklist match '${detail.source_url}' as listing '${listingId}'.`);
    return;
  }
