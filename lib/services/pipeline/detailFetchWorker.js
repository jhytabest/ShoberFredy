...
import { cancelWork, completeWork, registerHandler, retryWork, startWorker } from './workQueue.js';
import {
...
async function captureDetail(detail, providers, signal) {
  ...
  if (!job) {
    cancelWork('detail', detail.id, 'Job no longer exists');
    return;
  }
...
async function processDetail(detail, providerConfig, browser, signal) {
  ...
  if (discoveryReasons.length) {
    ...
    const sourceHash = captureVersionHash(detail.provider, detail.source_key, {
  ...
  if (representative) {
    ...
    if (merged.hiddenReason && merged.listingId) {
      cancelWork('detail', detail.id, merged.hiddenReason);
    } else {
