...
import { buildEvidence } from './listingLlmParser.js';
...
import { isProviderPaused, pausedForMs, recordProviderSignal } from '../jobs/providerCircuitBreaker.js';
...
async function captureDetail(detail, providers, signal) {
  ...
  const capture = needsBrowser
    ? await withBrowserSession(runtimeConfig.url || detail.source_url, { signal, closeAfter: true }, (browser) =>
        captureEvidence(detail, runtimeConfig, browser, signal),
      )
    : await captureEvidence(detail, runtimeConfig, null, signal);
  await processCapture(detail, capture, signal, job.market);
}

function classifyDetailFailure(item, classified) {
  if (classified.kind !== 'inactive') return null;
  const detail = toDetailRow(item);
  recordSourceUnavailable(detail.id, classified.message);
...
async function captureEvidence(detail, providerConfig, browser, signal) {
  ...
  let capture = await providerConfig.captureDetails(target, browser, signal);
  signal?.throwIfAborted();
...
async function processCapture(detail, capture, signal, market) {
  signal?.throwIfAborted();
  const deterministic = capture.inactiveReason ? null : extractDeterministicDetail(capture);
  const images = capture.inactiveReason ? [] : await downloadAndOptimizeImages(capture.images || [], { signal });
  signal?.throwIfAborted();
  ...
  if (capture.inactiveReason) {
    recordSourceUnavailable(detail.id, capture.inactiveReason);
  ...
  const representative = findDetailRepresentative(detail, dedupeKeys, sha256(buildEvidence(capture)));
  if (representative) {
  ...
  const captureKey = enqueueCapture({
    ...
    cardRejectionJobId: detail.card_rejection_job_id,
  });
