...
async function processDetail(detail, providerConfig, browser) {
  ...
  const filterReasons = preLlmFilterReasons(capture, detail.discovery, job);
  if (filterReasons.length) {
    const hiddenReason = primaryFilterReason(filterReasons);
    const listingId = markPreLlmHidden(detail, sourceHash, capture, hiddenReason, filterReasons);
