...
function classifyDetailFailure(item, classified) {
  ...
  // A liveness probe carries the listing it is asking about, because a restored
  // listing has no source row to answer for it.
  markSourcesInactive(detail.id, classified.message, { listingId: detail.liveness_listing_id });
...
async function processDetail(detail, providerConfig, browser, signal) {
  ...
  if (capture.inactiveReason) {
    markSourcesInactive(detail.id, capture.inactiveReason, { listingId: detail.liveness_listing_id });
  ...
  // A liveness probe asks one question — is this advert still up — and the page
  // answering it is the whole answer. Enqueuing a capture here would re-extract
  // a listing already in the ledger, which is a paid LLM call to learn nothing.
  if (detail.livenessOnly) {
    completeWork('detail', detail.id, {
      action: 'captured',
      code: 'captured',
      patch: { livenessOnly: null, livenessListingId: null },
    });
