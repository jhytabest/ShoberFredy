...
/*
 * Conservative, pre-LLM evidence preparation. The untouched raw text stays in
 * the capture for audit; only exact portal chrome lines are removed from the
 * text sent to the LLM. This module never produces canonical listing facts.
 */
...
export function cleanEvidenceText(rawText) {
  const output = [];
  let previous = null;
...
export function detectInactiveReason(rawText, provider) {
