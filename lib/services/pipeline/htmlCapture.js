...
export async function captureHtmlListing(listing, browser, options) {
  ...
  // Offline fixtures and custom executors may still return raw HTML. Keep the
  // established capture path as a compatibility fallback.
  if (typeof extracted === 'string') return captureHtmlString(listing, extracted, options);
