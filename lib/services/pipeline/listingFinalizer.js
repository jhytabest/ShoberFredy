...
export async function finalizeLive(queue, llm, { allowMissingCoordinates = false, signal } = {}) {
  ...
  // user. A filtered result must attach to that owner's existing canonical row
  // instead of creating a second hidden listing merely because the job differs.
  const exactSourceListing = findOwnerListingForParsingQueue(queue.id, queue.job_id);
  if (
    exactSourceListing &&
    (listing.hidden_reason || (!exactSourceListing.manually_deleted && !exactSourceListing.hidden_reason))
  ) {
 ...
 */
export async function finalizeBackfill(queue, llm, { allowMissingCoordinates = false, signal } = {}) {
  signal?.throwIfAborted();
  const db = SqliteConnection.getConnection();
  ...
  if (job) {
    // `existing` lets an unchanged address reuse the coordinate it already has.
    await attachCoordinates(candidate, job, allowMissingCoordinates, signal, existing);
  }
  signal?.throwIfAborted();
  ...
  // The live path keeps distance_to_destination current; the backfill path used
  // to leave it null, starving the market model and scorer of the field.
  if (job) attachDistance(existing.id, candidate, job);
