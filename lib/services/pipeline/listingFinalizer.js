...
function buildCanonical(queue, llm) {
  ...
  // `price` and `priceType` describe one number together, and `priceType` names
  // the basis `price` is on rather than what the advert's headline happened to
  // quote. That is already what every consumer assumes — `coldEquivalentRent`
  // reads `price` back as a cold rent whenever the type says 'cold' — so the two
  // must be derived in one step or the model silently scores a Warmmiete as a
  // Kaltmiete. Believing the stated type over the stated numbers was also its own
  // bug: `price_type: 'cold'` with a null cold rent and a stated warm one yielded
  // no price at all, and the listing was refused for `no_price`.
  const basis = coldEquivalentRent(
    {
      ...
      priceType: rent.price_type || 'unknown',
    },
    null,
  );
  // Warm is the fallback, not an equal alternative. An advert that states only a
  // Warmmiete and no charges cannot be put on a cold basis without inventing a
  // Nebenkosten ratio, and warm ≥ cold, so filtering it against maxPrice can only
  // refuse a flat that might have passed — never admit one that should not.
  const price = basis.rent ?? warmRent;
  const priceType = basis.rent != null ? basis.type : warmRent != null ? 'warm' : 'unknown';
