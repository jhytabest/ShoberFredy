...
import { attachSourcesToListing, sourceLinksForParsingQueue } from './sourceAudit.js';
...
export async function finalizeLive(queue, llm, { allowMissingCoordinates = false } = {}) {
  ...
  if (stored) {
    ...
    enqueueRating(stored.id, job.id, queue.provider, { notify: !listing.hidden_reason });
  ...
  if (duplicate) {
    attachSourcesToListing(queue.id, duplicate.id, listing.hidden_reason, 'final');
...
export async function finalizeBackfill(queue, llm, { allowMissingCoordinates = false } = {}) {
  ...
  candidate.hidden_reason = job ? visibilityVerdict(candidate, job) : null;
...
function visibilityVerdict(listing, job) {
  if (
    isOneOf(listing.title, job.blacklist || []) ||
    isOneOf(blacklistEvidenceText(listing.description), job.blacklist || [])
  )
    return 'blacklist';
...
function replaceCanonical(db, existing, candidate) {
  const manuallyDeleted = Boolean(candidate.hidden_reason || (existing.manually_deleted && !existing.hidden_reason));
  db.transaction(() => {
    db.prepare(
      `UPDATE listings
...
function refreshSimilarityIdentity(existing, candidate) {
  ...
  if (hasSemanticIdentity(candidate)) {
