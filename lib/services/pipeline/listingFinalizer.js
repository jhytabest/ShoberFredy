...
export async function finalizeLive(queue, llm, { signal } = {}) {
  ...
  await attachCoordinates(listing, signal, anchorCity, { queueId: queue.id, listingId: queue.listing_id ?? null });
  signal?.throwIfAborted();
  ...
  return db
    .prepare(
      `SELECT DISTINCT content_hash
       FROM listing_images
       WHERE queue_id = ? AND download_status IN ('stored', 'expired') AND content_hash IS NOT NULL`,
    )
    .all(queueId)
...
async function attachCoordinates(listing, signal, city, audit) {
  if (!listing.address) return;
  signal?.throwIfAborted();
  const coords = await geocodeAddress(listing.address, { city, signal, audit });
  signal?.throwIfAborted();
  if (coords == null) {
    throw new GeocodeDeferredError('Google geocoding is unavailable');
  }
