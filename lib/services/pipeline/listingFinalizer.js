...
export async function finalizeLive(queue, llm, { allowMissingCoordinates = false, signal } = {}) {
  ...
  if (listingId) {
    ...
    enqueueNotification(listingId, accepted[0].job, queue.provider);
    return { status: 'duplicate', listingId };
  }
  ...
  if (resemblance) {
    ...
    enqueueNotification(mergedId, accepted[0].job, queue.provider);
    return { status: 'duplicate', listingId: mergedId };
  }
  ...
  enqueueNotification(storedId, accepted[0].job, queue.provider);
