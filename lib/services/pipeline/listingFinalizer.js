...
import { canonicalFilterReasons, primaryFilterReason, primaryFilterStage } from './listingFilters.js';
import { canonicalFacts } from './stageFacts.js';
...
export async function finalizeLive(queue, llm, { allowMissingCoordinates = false, signal } = {}) {
  ...
  const listingId = identity ? absorbIntoExisting(db, queue, listing, identity, claims) : null;

  // Then the verdict, once per interested job over the one shared extraction.
  const facts = canonicalFacts(listing);
  ...
  if (resemblance) {
    const mergedId = absorbIntoExisting(db, queue, listing, resemblance, claims);
...
function recordAllVerdicts(db, listingId, verdicts) {
  for (const { job, reasons, reason, stage } of verdicts) {
    recordVerdict(db, {
      listingId,
      jobId: job.id,
 ...
 */
function absorbIntoExisting(db, queue, listing, match, claims) {
