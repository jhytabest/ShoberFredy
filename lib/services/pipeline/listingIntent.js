 ...
 * `furnished` and `furnishing_status` are validated enum fields on every
 * canonical listing — but nothing consulted them, so the decision kept being
...
const INTENT_RULES = [
  ...
  },
  {
    ...
    // `furnished` is only true for a full furnishing; the part-furnished case
    // lives in furnishingStatus, and the configured terms cover both.
    matches: (a) => a.furnished === true || a.furnishingStatus === 'full' || a.furnishingStatus === 'partial',
  },
  {
    intent: 'fixed_term',
