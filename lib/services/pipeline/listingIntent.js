...
const INTENT_RULES = [
  ...
  },
  {
    ...
    label: 'aggregator or relisting platform',
    // The blacklist named two of these by hand. The structured field names the
    // whole class, which is what the terms were reaching for — a portal name in
    // a keyword list is a proxy for "this is not the landlord".
...
export function intentFilterReasons(attributes, blacklist) {
  ...
  for (const rule of intents) {
    if (rule.matches(attributes)) {
      reasons.push({ code: 'intent', stage: 'extraction', intent: rule.intent, field: 'llm_structured' });
    }
  }
  return reasons;
}
