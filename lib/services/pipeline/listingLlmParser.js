 ...
 */
export async function analyzeImages(images, { budgetKind = 'live', audit = {}, signal } = {}) {
 ...
 */
export async function parseListingWithLlm({ capture, visual, budgetKind = 'live', audit = {}, signal }) {
  const startedAt = Date.now();
  const model = process.env.FREDY_LLM_TEXT_MODEL || 'nvidia/nemotron-3-ultra-550b-a55b:free';
  const system = systemPrompt();
  const evidence = buildEvidence(capture, visual, { includeHistorical: budgetKind === 'backfill' });

  let result = await callListingTool(model, system, evidence, [], budgetKind, audit, 'text_initial', signal);
  ...
  if (validation.valid && budgetKind === 'backfill') {
    const completenessIssues = historicalCompletenessIssues(capture, normalized.listing);
        ...
        [
          {
            role: 'user',
            content: [
              'Your structure is valid, but you left fields null despite labeled preserved evidence:',
              ...completenessIssues.map((issue) => `- ${issue}`),
              'Re-extract the complete listing. Validate each preserved value against all evidence; populate it when plausible and not contradicted. If the headline price type is unknown, put the supported amount in rent.cold or rent.warm and keep rent.price_type unknown. If evidence genuinely conflicts, keep the better-supported value and record the conflict.',
            ].join('\n'),
          },
        ],
        budgetKind,
 ...
 */
async function callListingTool(model, system, evidence, extraMessages, budgetKind, audit, operation, signal) {
...
const INSTRUCTIONS = [
  ...
  'For labeled historical backfill evidence, a non-null preserved scalar is valid evidence: validate it against the page/API evidence and populate the corresponding field when plausible and not contradicted. Do not leave it null merely because the detail page omitted it.',
];
...
function buildEvidence(capture, visual, { includeHistorical = false } = {}) {
  const maxText = positiveEnv('FREDY_LLM_MAX_TEXT_CHARS', 24000);
  const maxEmbedded = positiveEnv('FREDY_LLM_MAX_EMBEDDED_CHARS', 24000);
  const maxHistorical = positiveEnv('FREDY_LLM_MAX_HISTORICAL_CHARS', 8000);
  const sections = [
  ...
  if (includeHistorical) {
    sections.push(
      [
        ...
        'These are preserved facts captured when the listing was discovered. Use them as valid evidence when the detail text omits a field. Prefer explicit detail/API evidence when sources conflict, and record the conflict instead of silently combining incompatible values.',
        `DISCOVERY CARD:\n${clip(JSON.stringify(capture.discoveryData || {}), maxHistorical)}`,
        `LEGACY SNAPSHOT:\n${clip(JSON.stringify(capture.legacySnapshot || {}), maxHistorical)}`,
      ].join('\n\n'),
    );
  }
...
function historicalCompletenessIssues(capture, listing) {
  const issues = [];
  const preservedPrice = historicalNumber(capture, 'price');
  if (preservedPrice != null && listing?.rent?.cold == null && listing?.rent?.warm == null) {
    issues.push(`headline rent is null; preserved price evidence is ${preservedPrice} EUR`);
  }
  const preservedSize = historicalNumber(capture, 'size');
  if (preservedSize != null && listing?.size_sqm == null) {
    issues.push(`size_sqm is null; preserved size evidence is ${preservedSize} m²`);
  }
  const preservedRooms = historicalNumber(capture, 'rooms');
  if (preservedRooms != null && listing?.rooms == null) {
    issues.push(`rooms is null; preserved room-count evidence is ${preservedRooms}`);
  }
  return issues;
}

function historicalNumber(capture, key) {
  for (const evidence of [capture?.discoveryData, capture?.legacySnapshot]) {
