...
export async function parseListingWithLlm({ capture, audit = {}, signal }) {
  ...
  for (const [index, model] of models.entries()) {
    ...
    try {
      ...
      if (!validation.valid) {
        // Twice-invalid output is the model failing to speak the schema, which is
        // exactly what another model can fix. Let the loop reach the fallback
        // instead of retrying the same incompatible model until the listing dies.
...
export function validateEvidence(listing, capture) {
  ...
  if (
    listing.amenities.some((item) => item.name === 'new_building') &&
    !/neubau|new building|new construction|newly built|newly constructed/i.test(quotes('amenities.new_building'))
  ) {
    result.errors.push('new_building requires evidence of new construction; renovation alone is insufficient.');
  }
...
const DERIVATION_RULES = [
  ...
  'Do the arithmetic the advert lays out: Kaltmiete plus every recurring charge it lists, heating included, is the ' +
    'Warmmiete. Never work backwards from a Warmmiete to a Kaltmiete, and never invent a charge to complete a sum.',
];
