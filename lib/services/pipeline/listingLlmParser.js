...
function normalizeMechanicalOutput(value) {
  ...
  const repairs = [];
  for (const [path, values, identity] of [
    ['amenities', listing.amenities, (amenity) => amenity?.name],
    ['requirements', listing.requirements, (item) => item],
    ['conflicts', listing.conflicts, (item) => item],
    ['rent.included', listing.rent?.included, (item) => item],
  ]) {
...
const INSTRUCTIONS = [
  ...
  'State both rent.cold and rent.warm whenever the advert supports both: subtract the Nebenkosten, and any separately quoted Heizkosten, from a stated Warmmiete, or add them to a stated Kaltmiete. This is arithmetic on stated figures, not an estimate — if the advert gives only one figure and no charges, leave the other null. price_type names which figure the headline quotes, not which one you managed to fill.',
  ...
  'Populate the extended address, rent, lease, policy, requirements, and conflict fields whenever evidence supports them.',
