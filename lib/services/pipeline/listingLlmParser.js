...
const INSTRUCTIONS = [
  ...
  'State both rent.cold and rent.warm whenever the advert supports both: subtract the Nebenkosten, and any separately quoted Heizkosten, from a stated Warmmiete, or add them to a stated Kaltmiete. This is arithmetic on stated figures, not an estimate — if the advert gives only one figure and no charges, leave the other null. Always fill rent.service_charges when the advert names the Nebenkosten anywhere, even in prose: it is the single field that decides whether a warm-only advert can be compared with every other listing. price_type names which figure the headline quotes, not which one you managed to fill.',
