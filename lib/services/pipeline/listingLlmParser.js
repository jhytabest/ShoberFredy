 ...
 */
export async function analyzeImages(images, { audit = {}, signal } = {}) {
  const startedAt = Date.now();
  const model = process.env.FREDY_LLM_VISION_MODEL || 'nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free';
  const maxImages = env('FREDY_LLM_VISION_MAX_IMAGES');
 ...
 *
 * @param {{capture: object, visual: object[]|null}} input
 * @returns {Promise<{model: string, listing: object, durationMs: number}>}
 */
export async function parseListingWithLlm({ capture, visual, audit = {}, signal }) {
  const startedAt = Date.now();
