...
// relisting it, and no structured field expressed that. Deliberately optional —
// adding a name to `required` invalidates every stored extraction and makes the
// parser re-buy each one.
...
export const listingTool = {
  ...
  function: {
    ...
    description:
      ...
      'Set `offered_by` to `relisting_platform` when the advert is placed by an intermediary ' +
      "that resells or sublets other people's flats (HousingAnywhere, Spotahome, Wunderflats, " +
      'Homelike, Nestpick and similar), `agency` for a Makler, `property_management` for a ' +
      'Hausverwaltung, and `private` when the landlord or current tenant is advertising directly.',
    parameters: {
      type: 'object',
      ...
      properties: {
        ...
        offered_by: { type: 'string', enum: OFFERED_BY },
        ...
        rent: {
          ...
          properties: {
            ...
            service_charges: nullableNumber(0, 10000, 'Nebenkosten per month, excluding heating when stated apart.'),
 ...
 *
 * `maxItems` bounds the parser's dominant failure: with the arrays unconstrained,
 * greedy decoding regularly emitted one value ten times over — 303 of the last
 * 400 extractions needed `rent.included` deduplicated — which is most of why a
 * successful extraction averaged 2853 completion tokens and 58 seconds against a
 * 120-second deadline. A vocabulary-length cap cannot be exceeded by a correct
 * answer, so it truncates only a runaway.
 *
 * `uniqueItems` would say it exactly, and cannot be used: xgrammar, the
 * constrained decoder behind the free NVIDIA endpoint, refuses to compile a
 * schema containing it and rejects every request outright. Uniqueness is stated
 * in the instructions and enforced after the fact instead, which is why the
 * deduplicating normalizer still exists.
 */
