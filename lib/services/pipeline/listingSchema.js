    ...
    parameters: {
      ...
      properties: {
        ...
        rent: {
          ...
          properties: {
            // Two figures, each stated only when the advert states it. There is no
            // arithmetic to do: `service_charges` and `heating_costs` existed so a
            // warm-only advert could be turned into a cold one, and that
            // conversion reached two listings out of 2,098 while costing two
            // extraction fields, a consistency rule over them, and a flag in both
            // market models. A warm-only advert is simply a warm-only advert.
            //
            // `price_type` went with them: it named which figure the headline
            // quoted, and every consumer wanted to know which figure it *got*.
            // That is now read off the two values instead of asked for a third
            // time and believed over them — which was its own bug, since a stated
            // 'cold' with a null cold rent yielded no price at all.
            cold: nullableNumber(
              0,
              50000,
