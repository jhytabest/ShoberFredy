...
/** Matches the detail worker's wait, for the same reason: only an operator clears it. */
const PROXY_WAIT_MS = 30 * 60 * 1000;

/**
 * Ask one question about one listing: is this advert still up?
 *
 * Its own work kind, because it is not a detail capture. A detail item is an
 * advert some job discovered, and `captureDetail` resolves the job, the provider
 * configuration and the prior verdict by reading `listing_sources` — three gates
 * that all answer "whose card was this". A liveness probe has no card and no job,
 * and for the 1,278 listings restored from the pre-migration snapshot no source
 * row either, so every one of those gates answers "nobody" and cancels the probe
 * before a page is fetched. Riding the detail path meant `state = 'gone'` could
 * never be written for exactly the listings the pass exists to check.
 *
 * What it does need is much smaller: a URL, a provider module, and somewhere to
 * put the answer. All three come off the listing.
 *
...
export function startLivenessWorker({ providers }) {
  registerHandler(WORK_KIND, {
    name: WORKER_NAME,
    // Enqueued by scheduled maintenance, so it shares that switch: turning upkeep
    // off should not leave a worker polling for work nothing will produce.
    enabled: () => env('FREDY_MAINTENANCE_ENABLED'),
    timeoutMs: () => env('FREDY_DETAIL_ITEM_TIMEOUT_MS'),
    maxAttempts: () => env('FREDY_LIVENESS_MAX_FAILURES'),
...
/**
 * A provider saying the advert is gone is the answer, not a failure. Recorded
 * here because the shared policy cannot know which listing was being asked about.
 */
function classifyLivenessFailure(item, classified) {
  ...
  // No log here: `settleOutcome` carries severity 'info', and the worker loop
  // reports settled outcomes on its own. Saying it twice was the old habit.
...
async function probeListing(item, providers, signal) {
  ...
  if (!provider || typeof provider.config?.captureDetails !== 'function') {
    // Restored listings include portals no longer configured. Nothing can check
    // them, and saying so is better than retrying until the attempts run out.
  ...
  // The provider's own capture is reused rather than reimplemented: it already
  // knows each portal's "no longer available" shape, and that is the whole
  // question. Nothing is stored — no images, no text, no parse item — because a
  // probe is not an extraction and the listing is already in the ledger.
  ...
  // No `action`: `markListingAlive` already audited the listing-scoped fact, and
  // repeating it here only writes the same word twice under two queue ids.
