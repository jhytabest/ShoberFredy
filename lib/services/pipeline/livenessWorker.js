...
function toLivenessRow(item) {
  ...
  return {
    listingId: payload.listingId ?? item?.key,
...
async function probeListing(item, providers, signal) {
  const { listingId, provider: providerId, link, market } = toLivenessRow(item);
  ...
  if (isProviderPaused(providerId, market)) {
    deferWork(
      WORK_KIND,
      item.key,
      `'${providerId}' is paused after repeated failures`,
      Date.now() + pausedForMs(providerId, market),
      {
  ...
  const capture = needsBrowser
    ? await withBrowserSession(link, {}, (browser) => provider.config.captureDetails(target, browser))
  ...
  recordProviderSignal({ provider: providerId, market, scope: 'item', signal: 'ok' });
