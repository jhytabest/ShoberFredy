...
export function noteUpstreamExhausted(resetAtMs, now = Date.now()) {
  const until = clampReset(resetAtMs, now) ?? nextUtcDayStart(now);
...
function dailyLimit() {
  const parsed = Number.parseInt(process.env.FREDY_LLM_DAILY_LIMIT || '', 10);
...
function clampReset(resetAtMs, now) {
  ...
  return Math.min(Math.max(resetAtMs + RESET_MARGIN_MS, now + 1000), now + 26 * 60 * 60 * 1000);
}
