...
async function dispatchDeliveries(items) {
  {
    ...
    for (const group of groupDeliveries(rows).values()) {
      ...
      try {
        ...
        // Error, not warn: this fires once the item has exhausted every attempt
        // and is about to be dead-lettered, which is a listing the user will
        // never see. It was a warning, and the homeserver routes only critical
        // events, so 230 permanently undelivered notifications reached nobody.
        logger.event(
