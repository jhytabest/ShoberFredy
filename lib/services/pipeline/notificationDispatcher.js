...
import { notificationSettled } from './terminalVerdict.js';
...
async function dispatchDeliveries(items, signal) {
  {
    ...
    for (const group of groupDeliveries(rows).values()) {
      ...
      const visible = group.rows.filter(
        (row) =>
          !notificationSettled(SqliteConnection.getConnection(), row.id, job.id) &&
          decideListing(SqliteConnection.getConnection(), row, job).accepted,
      );
        ...
        visible.map(async (row) => {
          try {
            ...
            await telegram.send({
              ...
              jobKey: group.jobId,
              signal,
              deliveredChatIds: receiptsForDelivery(row.delivery_key),
              onReceipt: (chatId, response) =>
                recordDeliveryReceipt(row.delivery_key, row, group.jobId, chatId, response),
            });
