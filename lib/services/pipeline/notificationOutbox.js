...
export function markDeliveriesSent(listingIds) {
  for (const listingId of listingIds) {
    completeWork('notify', listingId, { status: 'sent', action: 'sent' });
    auditDelivery(listingId, 'sent');
  }
}

export function markDeliveriesCancelled(listingIds, reason) {
  for (const listingId of listingIds) {
    cancelWork('notify', listingId, reason);
...
export function markDeliveriesFailed(listingIds, error) {
  for (const listingId of listingIds) {
    retryWork('notify', listingId, error, { stage: 'notification' });
    auditDelivery(listingId, 'retry', String(error?.message || error));
  }
