...
import { markVerdictNotified, notificationSettled } from './terminalVerdict.js';
...
export function enqueueNotification(listingId, job, provider) {
  ...
  if (notificationSettled(SqliteConnection.getConnection(), listingId, job.id)) {
...
export function receiptsForDelivery(key) {
  return SqliteConnection.getConnection()
    .prepare('SELECT target_chat_id FROM notification_receipts WHERE delivery_key = ?')
    .all(key)
    .map((row) => row.target_chat_id);
}

export function recordDeliveryReceipt(key, row, jobId, targetChatId, response) {
  const receipt = response?.result;
  if (!Number.isInteger(receipt?.message_id) || receipt?.chat?.id == null) {
    throw new Error('Telegram returned no message receipt');
  }
  SqliteConnection.withTransaction((db) => {
    db.prepare(
      `INSERT OR IGNORE INTO notification_receipts
      (delivery_key, target_chat_id, listing_id, job_id, chat_id, message_id, received_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)`,
    ).run(key, targetChatId, row.id, jobId, String(receipt.chat.id), receipt.message_id, Date.now());
    recordListingAudit(row.id, {
      queueId: key,
      stage: 'notification',
      action: 'receipt',
      payload: { jobId, targetChatId, chatId: receipt.chat.id, messageId: receipt.message_id },
    });
  });
}
...
export function markDeliveriesSent(listingIds) {
  SqliteConnection.withTransaction((db) => {
    for (const key of listingIds) {
      const row = db.prepare("SELECT payload_json FROM pipeline_work WHERE kind = 'notify' AND key = ?").get(key);
      if (!row) continue;
      const payload = JSON.parse(row.payload_json);
      completeWork('notify', key, { status: 'sent', action: 'sent', code: 'delivered' });
      markVerdictNotified(payload.listingId ?? key, payload.jobId);
      auditDelivery(key, 'sent');
    }
  });
}
...
function auditDelivery(key, action, reason = null) {
  ...
  const listingId = data.listingId ?? key;
  recordListingAudit(listingId, {
    queueId: key,
    stage: 'notification',
    action,
