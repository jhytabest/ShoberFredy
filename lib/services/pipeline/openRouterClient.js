...
export async function openRouterToolCall({
  model,
  messages,
  tool,
  signal,
  ...
  const request = {
    ...
    max_tokens: env('FREDY_LLM_MAX_OUTPUT_TOKENS'),
    temperature: 0,
    reasoning: { enabled: false },
  };
  let auditId;
  try {
    auditId = beginLlmAudit({ context: audit, model, toolName: tool.function.name, request });
  } catch (error) {
    refundLlmCall(reservedAt);
    error.infrastructureFailure = true;
    throw error;
  }
  ...
  try {
    ...
    if (response.status === 429) {
      const reset = parseResetMs(response, responseBody);
      // Only platform quota headers justify stopping every model. Provider
      // capacity errors should reach the next model, even when returned as 429.
      if (response.headers.has('x-ratelimit-limit') || response.headers.has('x-ratelimit-remaining')) {
        const until = noteUpstreamExhausted(reset ?? Date.now() + env('FREDY_LLM_UPSTREAM_BACKOFF_MS'), {
          fromUpstream: true,
        });
        throw new LlmBudgetExhaustedError('OpenRouter account request limit reached', until);
      }
      const error = new LlmCallError('Extraction provider is rate limited', { outcome: 'rate_limited' });
      error.fallbackEligible = true;
      error.retryAtMs = reset;
      throw error;
    }
    if (!response.ok) {
      ...
      error.fallbackEligible = [404, 408, 422].includes(response.status) || response.status >= 500;
      if ([401, 402, 403].includes(response.status)) {
        error.retryAtMs = noteUpstreamExhausted(Date.now() + env('FREDY_LLM_UPSTREAM_MAX_BACKOFF_MS'));
      }
      throw error;
    }
    ...
    if (error.infrastructureFailure && !(error instanceof LlmBudgetExhaustedError)) {
      error.retryAtMs = pauseModel(model, outcome, Date.now(), error.retryAtMs);
    }
    ...
    // Refund only the local answer budget. An unanswered or aborted request may
    // still consume upstream quota/compute; external cost is known only from usage.
    if (UNANSWERED_OUTCOMES.has(outcome)) {
      refundLlmCall(reservedAt);
      budgetRefunded = true;
    }
    throw error;
