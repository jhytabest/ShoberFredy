...
export async function openRouterToolCall({ model, messages, tool, signal, audit = {} }) {
  ...
  try {
    ...
    if (error.infrastructureFailure && !(error instanceof LlmBudgetExhaustedError)) {
      error.retryAtMs = pauseModel(model, outcome);
    }
    auditError = error.message;
    error.fallbackEligible ??= ['timeout', 'transport_error', 'missing_tool_call'].includes(outcome);
    // The model never answered, so nothing was spent on it. Outcomes where it did
    // answer — badly — keep their charge: those cost the provider real tokens.
    if (UNANSWERED_OUTCOMES.has(outcome)) refundLlmCall(reservedAt);
    throw error;
  } finally {
