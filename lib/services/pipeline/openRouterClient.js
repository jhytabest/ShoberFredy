...
export async function openRouterToolCall({ model, messages, tool, signal, audit = {} }) {
  ...
  const request = {
    ...
    // OpenRouter fans a model id out across upstream providers and silently
    // drops parameters the chosen one does not implement. A provider without
    // tool support therefore answered a *forced* tool call with ordinary prose —
    // 415 extractions, 8% of every call ever made, failed as
    // 'missing_tool_call' for that reason alone. Requiring the parameters makes
    // OpenRouter route only to providers that honour tools and tool_choice, and
    // answer 404 rather than silently degrading when none is free. A 404 is
    // retried; prose is a dead end.
    provider: { require_parameters: true },
    temperature: 0,
    // Thinking off. `exclude: true` only hid the reasoning tokens from the
    // response — they were still generated and paid for in latency, which is
    // most of why a successful extraction averaged 68s and 17% of calls hit the
    // 120s deadline outright. This is a forced single tool call against a fixed
    // schema over supplied evidence: there is no multi-step reasoning for the
    // model to do that the schema does not already encode.
  ...
  // A 200-wrapped error that waiting cannot fix. Left to fall through, it was
  // read as "the model declined to call the tool", which is the one diagnosis
  // that fits every cause and points at none: a schema the provider's decoder
  // refused to compile spent an hour looking like a busy upstream, with the
  // queue deferring politely and /health reporting ok throughout.
  if (payload?.error) {
...
/**
 * A rejection of the request itself, which every retry will earn again. These
 * arrive wrapped in a provider's 5xx code — xgrammar, the constrained decoder
 * behind the free NVIDIA endpoint, reports an uncompilable tool schema as a
 * ValueError under code 502 — so the code alone cannot be trusted to mean "busy".
 */
