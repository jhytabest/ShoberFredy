...
/**
 * Extract the most reliable reset time from a 429 response: the
 * `X-RateLimit-Reset` header/body metadata is an epoch-ms timestamp,
...
function parseResetMs(response, body) {
  ...
  if (Number.isFinite(headerReset) && headerReset > 0) return headerReset;
  const bodyReset = Number(/"X-RateLimit-Reset"\s*:\s*"?(\d+)"?/.exec(body)?.[1]);
  if (Number.isFinite(bodyReset) && bodyReset > 0) return bodyReset;
