      ...
      if (jobs.length) {
        recordSourceRejection(
          { id: queue.id, provider: queue.provider, source_key: queue.source_key, source_url: queue.source_url },
          {
            ...
            // capture could change that — which is a different card evidence
            // hash, so the advert reopens exactly when the card does.
            evidenceKind: 'card',
            evidenceHash: cardEvidence(new CardFacts(queue.capture?.discoveryData)),
