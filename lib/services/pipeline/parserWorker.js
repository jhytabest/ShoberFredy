...
import { buildEvidence, parseListingWithLlm } from './listingLlmParser.js';
...
async function processQueueItem(queue, { signal } = {}) {
  ...
  if (!finalListing) {
    ...
    if (reused && extractionEnvelope(reused.llm_json)) {
      finalListing = extractionEnvelope(reused.llm_json);
