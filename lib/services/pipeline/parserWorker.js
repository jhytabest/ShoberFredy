...
import { CardFacts, GeoFacts } from './listingFilters.js';
...
async function processQueueItem(queue, { signal } = {}) {
  ...
  if (!finalListing) {
    ...
    const evidence = {
      ...
      geo: geoEvidence(new GeoFacts(null, queue.capture?.discoveryData)),
    };
    ...
    if (jobs.length && decisions.every((decision) => decision.decided)) {
      cancelWork('parse', queue.id, decisions[0].reason);
      ...
      if (jobs.length) {
        recordSourceRejection(
          { id: queue.id, provider: queue.provider, source_key: queue.source_key, source_url: queue.source_url },
          {
            ...
            evidenceKind: 'geo',
            evidenceHash: geoEvidence(new GeoFacts(null, queue.capture?.discoveryData)),
