...
async function superviseLoop() {
  ...
  while (!stopped) {
    try {
      ...
      if (!stopped) await delay(restartDelayMs);
    }
  }
}
 ...
 */
export async function processQueueItem(queue, { signal } = {}) {
  ...
  try {
    ...
    if (!finalListing && budgetKind === 'live' && visionEnabled() && !extraction?.vision_model) {
      try {
        const vision = await analyzeImages(getQueueImages(queue.id), { budgetKind, audit, signal });
    ...
    if (!finalListing) {
      const parsed = await parseListingWithLlm({ capture: queue.capture, visual, budgetKind, audit, signal });
  ...
  try {
    ...
    const result =
      queue.queue_kind === 'backfill'
        ? await finalizeBackfill(queue, finalListing, { allowMissingCoordinates, signal })
        : await finalizeLive(queue, finalListing, { allowMissingCoordinates, signal });
    signal?.throwIfAborted();
...
function parserItemTimeoutMs() {
  return positiveEnv('FREDY_PARSER_ITEM_TIMEOUT_MS', 180_000);
}
