...
import { canExtract, LlmBudgetExhaustedError } from './llmBudget.js';
...
export function startParserWorker() {
  registerHandler('parse', {
    ...
    classify: (item, classified, error) => {
      if (error.infrastructureFailure || error?.code === 'OPERATION_DEADLINE') {
        return parkOutcome(
          'transient',
          error.message,
          error.retryAtMs ?? Date.now() + env('FREDY_LLM_UPSTREAM_BACKOFF_MS'),
          { environmental: true },
        );
      }
      ...
      if (error instanceof GeocodeDeferredError) {
        return parkOutcome('geocode_unavailable', error.message, Date.now() + 60_000, {
...
async function processQueueItem(queue, { signal } = {}) {
  ...
  if (!finalListing) {
    ...
    if (!jobs.length) {
      cancelWork('parse', queue.id, 'No interested job remains', { code: 'no_interested_job' });
  ...
  try {
    ...
    signal?.throwIfAborted();
    if (error instanceof LlmBudgetExhaustedError) {
      deferWork('parse', queue.id, error.message, error.retryAtMs, { code: 'llm_budget', environmental: true });
  ...
  try {
    ...
    const result = await finalizeLive(queue, finalListing, { signal });
    signal?.throwIfAborted();
