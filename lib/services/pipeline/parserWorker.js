...
import { cardEvidence, filterConfigHash, geoEvidence, terminalVerdict } from './terminalVerdict.js';
...
export function startParserWorker() {
  registerHandler('parse', {
    ...
    // Parse keeps its own accounting for the two conditions it must act on at
    // the exact moment they run out — the LLM budget, which is counted apart
    // from the item's attempts, and the geocoder. What this hook covers is those
    // same conditions escaping the handler, so they reach the shared policy as
    // parks rather than as unclassified failures.
...
async function processQueueItem(queue, { signal } = {}) {
  ...
  if (!finalListing) {
    ...
    // The capture carries both kinds of evidence a cheaper stage could have
    // decided on, so both are offered and both are rechecked. Passing neither
    // made this call unable to fire at all.
    const evidence = {
      card: cardEvidence(new CardFacts(queue.capture?.discoveryData)),
      geo: geoEvidence(queue.capture?.discoveryData?.address),
    };
      ...
      if (jobs.length) {
        recordSourceRejection(
          { id: queue.id, provider: queue.provider, source_key: queue.source_key, source_url: queue.source_url },
          {
            ...
            stage: 'detail',
            // The model could not read this capture, and only a different
            // capture could change that — which is a different geo evidence
            // hash, so the advert reopens exactly when its address does.
            evidenceKind: 'geo',
            evidenceHash: geoEvidence(queue.capture?.discoveryData?.address),
