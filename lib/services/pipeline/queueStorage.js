...
import { canSpend } from './llmBudget.js';
import {
  attachParsingQueue,
...
export function enqueueDiscovery({ jobId, provider, listing }) {
  ...
  const result = db.transaction(() => {
    ...
    if (!existing) {
      const representative = findDiscoveryRepresentative(db, {
        jobId,
        provider,
        sourceKey,
        sourceUrl,
...
/**
 * Whether this exact pre-LLM verdict is already recorded against the queue row.
 *
 ...
 *
 * @param {object} detail hydrated detail_fetch_queue row
...
export function enqueueCapture({ jobId, provider, sourceHash, capture, images = [], listingId, detailQueueId }) {
  ...
  const effectiveQueueId = db.transaction(() => {
    const active = db
      .prepare(
        `SELECT * FROM parsing_queue
         WHERE job_id = ? AND provider = ? AND source_key = ?
           AND status IN ('pending', 'retry', 'processing')`,
      )
      .get(jobId, provider, sourceKey);
    if (active) {
      // There is only one unfinished semantic parse per stable provider ad,
      // even if discovery text or price changed its content hash.
      if (active.status !== 'processing') {
        db.prepare(
          `UPDATE parsing_queue
           SET listing_id = COALESCE(listing_id, ?), source_hash = ?, external_id = ?, source_url = ?,
    ...
    const result = db
      .prepare(
        `INSERT OR IGNORE INTO parsing_queue (
           id, job_id, provider, source_key, source_hash, listing_id,
    ...
    if (result.changes === 0) {
      const existing = db
        .prepare(
          `SELECT id, status, source_hash, listing_id FROM parsing_queue
           WHERE job_id = ? AND provider = ? AND source_key = ?`,
        )
        .get(jobId, provider, sourceKey);
      if (!existing || existing.source_hash === sourceHash) return existing?.id;
      resolvedQueueId = existing.id;
      db.prepare(
        `UPDATE parsing_queue SET listing_id = ?, source_hash = ?, external_id = ?, source_url = ?,
           discovered_at = ?, capture_json = ?, stage = 'captured', status = 'pending', attempt_count = 0,
        ...
        listingId ?? existing.listing_id ?? null,
        sourceHash,
        capture.externalId ?? null,
        capture.sourceUrl ?? null,
        capture.discoveredAt ?? now,
 ...
 *
 * Interrupted work is reclaimed first. Items that still need an LLM call
 * respect the persistent daily budget; cached finalization retries remain
 * claimable while that budget is exhausted.
 *
...
export function claimNext({ now = Date.now(), leaseMs = DEFAULT_LEASE_MS } = {}) {
  const db = SqliteConnection.getConnection();
  const budgetOk = canSpend(now).ok;
  return db.transaction(() => {
    const row = selectReady(db, now, budgetOk);
    if (!row) return null;
...
function selectReady(db, now, budgetOk) {
  return db
    .prepare(
    ...
    .get(now, now, budgetOk ? 1 : 0);
}

export function saveExtraction(queueId, patch) {
  const db = SqliteConnection.getConnection();
  db.transaction(() => {
    const queue = db.prepare('SELECT listing_id FROM parsing_queue WHERE id = ?').get(queueId);
    if (!queue) return;
...
export function retryQueue(
  ...
  db.transaction(() => {
    ...
    finishLatestAttempt(queueId, changed.changes ? 'retry' : 'cancelled', db);
    if (changed.changes) auditQueueEvent(db, queueId, 'parse', 'retry', error, { llm, ...classification });
  })();
...
export function deferQueue(queueId, reason, untilMs, { geocode = false } = {}) {
  ...
  db.transaction(() => {
    ...
    finishLatestAttempt(queueId, changed.changes ? 'deferred' : 'cancelled', db);
    if (changed.changes) auditQueueEvent(db, queueId, 'parse', 'deferred', reason, { geocode });
  })();
}
...
export function completeQueue(queueId, listingId, status = 'completed') {
  ...
  SqliteConnection.withTransaction((db) => {
    const queue = db
      .prepare("SELECT json_extract(capture_json, '$.fullText') AS full_text FROM parsing_queue WHERE id = ?")
      .get(queueId);
    const changed = db
      .prepare(
        `UPDATE parsing_queue
       SET listing_id = COALESCE(?, listing_id), status = ?, stage = 'completed',
           lease_until = NULL, completed_at = ?, updated_at = ?, capture_json = NULL
    ...
    if (listingId) {
      saveListingText(listingId, queue?.full_text, now, db);
    ...
    db.prepare(
      `UPDATE detail_fetch_queue SET capture_json = NULL, discovery_json = NULL
       WHERE capture_queue_id = ? AND status IN ('completed', 'inactive', 'cancelled')`,
    ).run(queueId);
