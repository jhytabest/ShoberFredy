...
export function getExtraction(key) {
  ...
  if (!row) return null;
  for (const column of ['visual_json', 'llm_json']) {
    row[column] = fromJson(row[column], null);
  }
  return row;
}

export function getQueueImages(key) {
  return SqliteConnection.query('SELECT * FROM listing_images WHERE queue_id = @key ORDER BY position ASC', { key });
}
