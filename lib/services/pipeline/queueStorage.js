...
import { isRecoverableParseFailure } from './parseRecovery.js';
...
export function enqueueCapture({
  ...
  db.transaction(() => {
    ...
    const previous = getWork('parse', key);
    const recover = isRecoverableParseFailure(previous);
    enqueueWork(
      'parse',
      key,
      {
      ...
      { mode: recover ? 'reset' : 'fingerprint', fingerprintKey: 'sourceHash' },
    );
    if (recover)
      auditWork(
        'parse',
        key,
        { action: 'recovered_from_fresh_capture', reason: previous.last_error ?? previous.outcome_note },
        db,
      );
...
function replaceQueueImages(db, key, listingId, images) {
  ...
  const insert = db.prepare(
    `INSERT INTO listing_images (
       ...
       content_hash, mime_type, byte_size, width, height, download_status, error, retained_until
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  );
  for (const image of images) {
    insert.run(
      ...
      image.error ?? null,
      Date.now() + env('FREDY_MEDIA_RETENTION_DAYS') * 86400000,
    );
  }
}
