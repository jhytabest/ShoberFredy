...
export function enqueueCapture({ jobId, provider, sourceHash, capture, images = [], queueKind = 'live', listingId }) {
  ...
  return db.transaction(() => {
    ...
    if (result.changes === 0) {
      ...
      if (!existing || !['dead', 'cancelled'].includes(existing.status)) return existing?.id;
 ...
 *
 * Live items always take precedence over backfill. Items that still need
 * LLM calls are only claimed while the corresponding budget (live or the
 * backfill share) has headroom; items whose extraction is already cached
 * (finalize-stage retries) are claimable regardless of budget. Reclaiming an
 * expired-lease 'processing' row counts as one failed attempt so an item
 * that crashes the process cannot loop forever.
 *
...
export function claimNext({ now = Date.now(), leaseMs = DEFAULT_LEASE_MS } = {}) {
  const db = SqliteConnection.getConnection();
  return db.transaction(() => {
    let row = selectReady(db, 'live', now, canSpend('live', now).ok);
    if (!row && !isBackfillPaused(db)) {
      row = selectReady(db, 'backfill', now, canSpend('backfill', now).ok);
    }
    if (!row) return null;
 ...
 * can wait for days without ever being dead-lettered.
 *
 * @param {string} queueId
