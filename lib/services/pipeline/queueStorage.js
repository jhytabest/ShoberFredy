...
export function toParseRow(item) {
  ...
  return {
    ...
    // The card refusal this advert was sampled against, or null for the 97% that
    // were never refused at all. Carried this far because the verdict is where it
    // is compared and then applied.
    card_rejection: payload.cardRejection ?? null,
...
export function enqueueDiscovery({ jobId, provider, listing }) {
  ...
  return db.transaction(() => {
    ...
    // Judged before the work is opened, on evidence that is free to read, so the
    // verdict can travel with the item.
    const reasons = cardFilterReasons(facts, job);
    // A card rejection is never fetched, never extracted and never revisited, so
    // no card-stage refusal has ever been checked against anything. 1,336 sources
    // a cycle are refused on evidence nothing can audit — including 174 matches on
    // the bare token `WG`, which also fires on a whole flat titled "3-Zi für
    // Studenten-WG".
    //
    // A small random sample is extracted anyway, so the card's answer can be held
    // against the sharper post-extraction equivalent of the same term: `möbliert`
    // against furnishing_status, `Tausch` against listing_type, `WG` against
    // wg_room. It buys one LLM call each and it changes nothing: the card's
    // refusal is what the verdict records either way (see `finalizeLive`). The
    // sample measures the filter; it does not overrule it.
