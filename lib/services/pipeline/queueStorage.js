...
import { cardFilterReasons, primaryFilterReason, primaryFilterStage } from './listingFilters.js';
import { cardFacts } from './stageFacts.js';
import { cardEvidenceHash, filterConfigHash, terminalVerdict } from './terminalVerdict.js';
...
export function enqueueDiscovery({ jobId, provider, listing }) {
  ...
  return db.transaction(() => {
    ...
    const evidenceHash = cardEvidenceHash(facts);
    const decided = terminalVerdict(db, {
      claims: cardClaims(sourceUrl, provider, sourceKey),
      job,
      tier: 'card',
      evidenceHash,
    });
      ...
      if (detail) {
        recordSourceRejection(detail, {
          ...
          facts: {
            ...
            price: facts.cardPrice,
            size: facts.cardSize,
            rooms: facts.cardRooms,
          },
          reasons,
        });
