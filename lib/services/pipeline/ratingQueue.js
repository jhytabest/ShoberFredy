...
export function stopRatingWorker() {
  stopped = true;
}
...
export function processRating(item) {
  ...
  if (item.notify && job && row.manually_deleted === 0 && !row.hidden_reason) {
    enqueueNotificationDeliveries(item.listing_id, job, item.provider);
  }
  const status = score ? 'completed' : 'unrated';
