...
export function markPreLlmHidden(detail, sourceHash, capture, reason, reasons = [{ code: reason, stage: 'pre_llm' }]) {
  ...
  return db.transaction(() => {
    ...
    // A row nobody can recognise again is a row that will be created again.
    // `findOwnerExactSourceListing` above already looks a rejection up by these
    // very claims — nothing ever wrote them for this path, so from the day
    // progressive filtering began rejecting listings before the LLM, every
    // rejection was invisible to dedupe and returned as a fresh row on the next
    // capture whose page text differed at all. Recording the same set the lookup
    // queries is what closes that loop.
    recordClaims(db, listing.id, identityClaims(sources), now);
