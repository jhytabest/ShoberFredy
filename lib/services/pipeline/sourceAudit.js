...
export function recordDiscoverySource(
  ...
  if (detailQueueId && !source.parsing_queue_id) {
    const sibling = db
      .prepare(
        `SELECT parsing_queue_id, listing_id FROM listing_sources
      WHERE detail_queue_id = ? AND id != ? AND parsing_queue_id IS NOT NULL
      ORDER BY last_seen_at DESC LIMIT 1`,
      )
      .get(detailQueueId, source.id);
    if (sibling) {
      db.prepare('UPDATE listing_sources SET parsing_queue_id = ?, listing_id = ? WHERE id = ?').run(
        sibling.parsing_queue_id,
        sibling.listing_id,
        source.id,
      );
      source = db.prepare('SELECT * FROM listing_sources WHERE id = ?').get(source.id);
  ...
  syncSourceIdentityKeys(db, source);
  if (source.listing_id) db.prepare('UPDATE listings SET last_seen_at = ? WHERE id = ?').run(now, source.listing_id);
...
export function findDiscoveryRepresentative(db, { provider, sourceKey, sourceUrl, dedupeKeys }) {
  ...
  return (
    db
      .prepare(
        `SELECT s.* FROM source_identity_keys k
    JOIN listing_sources s ON s.id = k.source_id
    WHERE k.identity_key IN (SELECT value FROM json_each(?))
      AND NOT (s.provider = ? AND s.source_key = ?) AND s.detail_queue_id IS NOT NULL
    ORDER BY s.first_seen_at, s.id LIMIT 1`,
      )
      .get(JSON.stringify(keys), provider, sourceKey) ?? null
  );
}

function syncSourceIdentityKeys(db, source) {
  const keys = parseJson(source.dedupe_keys_json || '[]');
  const url = canonicalUrl(source.source_url);
  if (url) keys.push(`url:${url}`);
  db.prepare('DELETE FROM source_identity_keys WHERE source_id = ?').run(source.id);
  const insert = db.prepare('INSERT OR IGNORE INTO source_identity_keys VALUES (?, ?)');
  for (const key of new Set(keys)) insert.run(key, source.id);
}
...
export function recordDetailCapture(detail, capture, dedupeKeys) {
  ...
  db.transaction(() => {
    ...
    for (const source of sources) {
      ...
      syncSourceIdentityKeys(db, db.prepare('SELECT * FROM listing_sources WHERE id = ?').get(source.id));
...
export function findDetailRepresentative(detail, dedupeKeys, evidenceHash) {
  if (!dedupeKeys.length) return null;
  const db = SqliteConnection.getConnection();
  const matchedKey = db
    .prepare(
      `SELECT s.*
       FROM source_identity_keys known JOIN listing_sources s ON s.id = known.source_id
       WHERE NOT (s.provider = ? AND s.source_key = ?)
         AND known.identity_key IN (SELECT value FROM json_each(?))
  ...
  const candidate = matchedKey ?? storedClaimRepresentative(db, detail, dedupeKeys);
  if (!candidate || !evidenceHash) return null;
  const extraction = db
    .prepare(
      `SELECT 1 FROM listing_extractions WHERE queue_id = ?
    AND json_extract(llm_json, '$.provenance.evidenceHash') = ?`,
    )
    .get(candidate.parsing_queue_id, evidenceHash);
  return extraction ? candidate : null;
}
...
export function recordSourceRejection(
  ...
  return db.transaction(() => {
    const allSources = db.prepare(`SELECT * FROM listing_sources WHERE ${column} = ?`).all(detail.id);
    const sources = allSources.filter((source) => jobId == null || source.job_id === jobId);
    ...
    if (jobId == null || allSources.every((source) => source.job_id === jobId))
...
export function attachSourcesToListing(parsingQueueId, listingId, reason = null, dedupeStage = null) {
  ...
  db.transaction(() => {
    ...
    for (const source of sources) {
      ...
      const decision = db
        .prepare('SELECT verdict, reason FROM listing_verdicts WHERE listing_id = ? AND job_id = ?')
        .get(listingId, source.job_id);
      const sourceReason = decision ? decision.reason : reason;
      audit(db, {
        sourceId: source.id,
        listingId,
        queueId: parsingQueueId,
        stage: dedupeStage === 'final' ? 'final_dedupe' : 'extraction_filter',
        action: dedupeStage === 'final' ? 'merged' : sourceReason ? 'rejected' : 'accepted',
        reason: sourceReason,
      });
    }
  })();
...
function jobsFor(column, queueId) {
  const rows = SqliteConnection.getConnection()
    .prepare(`SELECT DISTINCT job_id, provider FROM listing_sources WHERE ${column} = ? ORDER BY job_id`)
    .all(queueId);
  const jobs = rows
    .map((row) => ({ job: getJob(row.job_id), provider: row.provider }))
    .filter(({ job, provider }) => job?.enabled && job.provider.some((entry) => entry.id === provider))
    .map(({ job }) => job);
...
// An extraction is reusable only while it says what the current parser would say
// about the same evidence: a changed schema or prompt makes the stored answer a
// different answer, not a cheaper one.
export function reusableExtraction(db, parsingQueueId, claims, evidenceHash, { schemaVersion, promptVersion }) {
  const listingIds = [
    ...new Set(
      resolveClaims(db, claims)
    ...
    .prepare(
      `SELECT queue_id, llm_json, text_model FROM listing_extractions
       WHERE listing_id IN (SELECT value FROM json_each(?))
         ...
         AND json_extract(llm_json, '$.provenance.evidenceHash') = ?
         AND json_extract(llm_json, '$.provenance.schemaVersion') = ?
         AND json_extract(llm_json, '$.provenance.promptVersion') = ?
       ORDER BY parsed_at DESC LIMIT 1`,
    )
    .get(JSON.stringify(listingIds), parsingQueueId, evidenceHash ?? null, schemaVersion, promptVersion);
  if (!row) return null;
  try {
...
export function recordSourceUnavailable(detailQueueId, reason) {
  const db = SqliteConnection.getConnection();
  db.transaction(() => {
...
function recordObservation(db, sourceId, stage, contentHash, payload, observedAt) {
  ...
  db.prepare(
    `INSERT OR IGNORE INTO listing_source_observations (
       id, source_id, stage, content_hash, content_bytes, observed_at, payload_sha256
     ) VALUES (?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    nanoid(),
    sourceId,
    ...
    observedAt,
    storeAuditPayload(payload, db, observedAt),
  );
}
