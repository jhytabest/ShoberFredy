...
import { canonicalUrl } from './temporaryDeterministic.js';
...
export function findDetailRepresentative(detail, dedupeKeys) {
  ...
  const canonicalUrls = canonicalSourceUrls(urlKeys(dedupeKeys));
  if (!canonicalUrls.size) return null;
  return db
    .prepare(
      `SELECT s.*
...
export function markPreLlmHidden(detail, sourceHash, capture, reason, reasons = [{ code: reason, stage: 'pre_llm' }]) {
  ...
  return db.transaction(() => {
    ...
    listing ??= db
      ...
      .get(detail.id);
    listing ??= findOwnerExactSourceListing(db, detail.job_id, sources);
    ...
    if (!listing) {
      ...
      db.prepare(
        `INSERT INTO listings (
           ...
           hidden_reason, canonical_schema_version, source_urls_json, filter_reasons_json
         ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 1, ?, 0, ?, ?)`,
      ).run(
        listingId,
        sourceHash,
    ...
    db.prepare(
      `UPDATE detail_fetch_queue
       SET status = 'cancelled', lease_until = NULL, completed_at = ?, updated_at = ?, last_error = ?
       WHERE id = ?`,
    ).run(now, now, reason, detail.id);
    return listing.id;
  })();
}

/**
 * Resolve an exact provider URL to an existing canonical row owned by the same
 * user, including sources collected under a different search job.
 */
export function findOwnerListingForParsingQueue(parsingQueueId, jobId) {
  const db = SqliteConnection.getConnection();
  const sources = db.prepare('SELECT * FROM listing_sources WHERE parsing_queue_id = ?').all(parsingQueueId);
  return findOwnerExactSourceListing(db, jobId, sources);
}
...
function cancelListingWork(db, listingId, reason, now) {
  const queueIds = db
    .prepare(
      `SELECT id FROM parsing_queue WHERE listing_id = ?
       UNION SELECT parsing_queue_id FROM listing_sources
       WHERE listing_id = ? AND parsing_queue_id IS NOT NULL`,
    )
    .all(listingId, listingId)
    .map((row) => row.id);
  const detailIds = db
    .prepare(
      'SELECT DISTINCT detail_queue_id AS id FROM listing_sources WHERE listing_id = ? AND detail_queue_id IS NOT NULL',
    )
    .all(listingId)
    .map((row) => row.id);
  const cancelQueue = db.prepare(
    `UPDATE parsing_queue SET status = 'cancelled', lease_until = NULL, completed_at = ?, updated_at = ?, last_error = ?
     WHERE id = ? AND status IN ('pending', 'retry', 'processing')`,
  );
  for (const queueId of queueIds) cancelQueue.run(now, now, reason, queueId);
  const cancelDetail = db.prepare(
    `UPDATE detail_fetch_queue SET status = 'cancelled', lease_until = NULL, completed_at = ?, updated_at = ?, last_error = ?
     WHERE id = ? AND status IN ('pending', 'retry', 'processing')`,
  );
  for (const detailId of detailIds) cancelDetail.run(now, now, reason, detailId);
  db.prepare(
    `UPDATE rating_queue SET status = 'cancelled', lease_until = NULL, completed_at = ?, updated_at = ?, last_error = ?
     WHERE listing_id = ? AND status != 'cancelled'`,
  ).run(now, now, reason, listingId);
  db.prepare(
    `UPDATE notification_deliveries SET status = 'cancelled', last_error = ?
     WHERE listing_id = ? AND status = 'pending'`,
  ).run(reason, listingId);
}

function findOwnerExactSourceListing(db, jobId, sources) {
  const exactKeys = exactSourceKeys(sources);
  if (exactKeys.length) {
    const matchedKey = db
      .prepare(
        `SELECT listing.*
         FROM jobs current_job
         JOIN jobs source_job ON source_job.user_id = current_job.user_id
         JOIN listing_sources source ON source.job_id = source_job.id
         JOIN listings listing ON listing.id = source.listing_id
         JOIN json_each(source.dedupe_keys_json) known
         WHERE current_job.id = ?
           AND known.value IN (SELECT value FROM json_each(?))
         ORDER BY
           CASE WHEN listing.manually_deleted = 0 AND listing.hidden_reason IS NULL THEN 0 ELSE 1 END,
           listing.canonical_schema_version DESC,
           listing.created_at ASC,
           listing.id ASC
         LIMIT 1`,
      )
      .get(jobId, JSON.stringify(exactKeys));
    if (matchedKey) return matchedKey;
  }

  const canonicalUrls = canonicalSourceUrls([...sources.map((source) => source.source_url), ...urlKeys(exactKeys)]);
  if (!canonicalUrls.size) return null;
  return db
    .prepare(
      `SELECT listing.*, source.source_url AS identity_source_url
       FROM jobs current_job
       JOIN jobs source_job ON source_job.user_id = current_job.user_id
       JOIN listing_sources source ON source.job_id = source_job.id
       JOIN listings listing ON listing.id = source.listing_id
    ...
    .all(jobId)
    .find((listing) => canonicalUrls.has(canonicalUrl(listing.identity_source_url)));
}

function exactSourceKeys(sources) {
  const keys = [];
  for (const source of sources) {
    const stored = parseJson(source.dedupe_keys_json || '[]');
    if (Array.isArray(stored)) keys.push(...stored.filter((key) => String(key).startsWith('url:')));
