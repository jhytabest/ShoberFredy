 ...
 * @returns {void}
 */
export function markSourcesInactive(detailQueueId, reason, { listingId = null } = {}) {
  const db = SqliteConnection.getConnection();
  const now = Date.now();
  db.transaction(() => {
    const sources = db
    ...
    //
    // `listingId` is accepted because the sources are not always there to ask.
    // Listings restored from the pre-migration snapshot came with verdicts, texts
    // and claims but no sources at all — 1,278 of them — and they are exactly the
    // stale ones a liveness probe targets. Reading the answer only out of
    // `listing_sources` meant those could never be marked gone no matter how many
    // times a probe confirmed they were.
    const listingIds = [...new Set([...sources.map((source) => source.listing_id), listingId].filter(Boolean))];
