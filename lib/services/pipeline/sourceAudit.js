...
export function recordSourceRejection(
  detail,
  { reason, stage, evidenceKind, evidenceHash = null, configHash, captureHash = null, reasons = [], queue = 'detail' },
) {
  const db = SqliteConnection.getConnection();
  const now = Date.now();
  ...
  return db.transaction(() => {
    ...
    const upsert = db.prepare(
      `INSERT INTO source_rejections
         (source_id, reason, stage, config_hash, evidence_kind, evidence_hash, capture_hash,
          decided_at, decided_count, last_seen_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?)
       ON CONFLICT(source_id) DO UPDATE SET
         reason = excluded.reason, stage = excluded.stage, config_hash = excluded.config_hash,
    ...
    for (const source of sources) {
      upsert.run(source.id, reason, stage, configHash, evidenceKind, evidenceHash, captureHash, now, now);
      db.prepare(
        `UPDATE listing_sources
    ...
    // A refusal, not an error. The code is the filter's own vocabulary and the
    // readable account is already in source_rejections, so nothing about this
    // belongs in last_error — which is where it used to land, and why 1,161
    // healthy refusals read as a queue full of errors named 'blacklist'.
...
/**
 * The identity claims of the advert this detail item is about.
 *
 * @param {string} detailQueueId
 * @returns {{claim: string, kind: string}[]}
 */
...
/**
 * Record what a liveness probe found, against the listing itself.
 *
 * A probe is not a discovery: it has no card, no job and — for the 1,278 listings
 * restored from the pre-migration snapshot — no `listing_sources` row to speak
 * through. Everything the detail path reads to answer "whose advert is this" is
 * source-derived, so the answer for those listings is always "nobody", and that
 * is why `state = 'gone'` had never once been written. These two write to
 * `listings` directly because the listing is the only party that still exists.
 *
 * A wrong `gone` is self-correcting: `listingFinalizer` sets a listing back to
 * active whenever it is seen again, so the worst case for a cross-posted flat
 * whose other portal still carries it is one cycle marked gone.
 *
 * @param {string} listingId
 * @param {string} reason
 * @param {number} [now]
 * @returns {boolean} whether the listing moved to gone
 */
...
/**
 * The advert is still up. `last_seen_at` moves because confirming it is a
 * sighting — and because the liveness pass selects on that column, a probe that
 * did not move it would re-select the same listing on every pass forever and
 * never reach the rest of the backlog.
 *
 * @param {string} listingId
 * @param {number} [now]
 * @returns {void}
