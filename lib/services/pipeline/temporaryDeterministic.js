...
export function detailDedupeKeys({ discovery, capture, images = [] }) {
  const keys = discoveryDedupeKeys(discovery);
  const normalizedText = normalizeText(capture?.fullText);
  if (normalizedText.length >= 160) keys.push(`text:${sha256(normalizedText)}`);
