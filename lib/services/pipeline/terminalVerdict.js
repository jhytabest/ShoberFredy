...
// "Which listings count?" has three different answers in this codebase, and
// they used to be sixteen inline copies of `verdict = 'accepted'` that quietly
// disagreed about whether state mattered. They are distinct questions, so they
// get distinct names rather than one blurred predicate.
//
// Each is a correlated-subquery fragment; interpolate with the `listings` alias
...
// Accepted and still being advertised — what a human would call "on offer".
export const ACCEPTED_AND_ACTIVE_SQL = (listings = 'l') =>
  `EXISTS (SELECT 1 FROM listing_verdicts v
            WHERE v.listing_id = ${listings}.id AND v.verdict = 'accepted')
   AND ${listings}.state = 'active'`;

// Everything a notifying job looked at and did not refuse outright. Wider than
// accepted on purpose: the market model is fitted on the market, not on the
// subset that passed one search's filters.
export const IN_MARKET_CORPUS_SQL = (listings = 'l') =>
  `EXISTS (SELECT 1 FROM listing_verdicts v JOIN jobs j ON j.id = v.job_id
            WHERE v.listing_id = ${listings}.id
              AND j.notify IS NOT NULL
