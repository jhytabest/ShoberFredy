...
export function notificationSettled(db, listingId, jobId) {
  return Boolean(
    db
      .prepare(
        `SELECT 1 FROM listing_verdicts WHERE listing_id = @listingId AND job_id = @jobId AND notified_at IS NOT NULL
        UNION ALL SELECT 1 FROM notification_suppressions WHERE listing_id = @listingId AND job_id = @jobId LIMIT 1`,
      )
      .get({ listingId, jobId }),
  );
}
