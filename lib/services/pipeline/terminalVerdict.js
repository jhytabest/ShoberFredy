...
/**
 * The evidence a geography-shaped verdict was reached from: the stated address.
 * Two captures of one advert differ in page text constantly and in stated
 * address almost never.
 *
 * Takes the address itself. It used to take a `GeoFacts`, which existed to feed
 * the pre-extraction area filter; that filter is gone and the class with it, but
 * the hash is unchanged — `GeoFacts` only ever contributed its trimmed address.
 *
 * @param {string|null|undefined} address
 * @returns {string}
 */
export function geoEvidence(address) {
  return sha256(String(address ?? '').trim() || null);
}
 ...
 * @returns {void}
 */
export function recordVerdict(db, { listingId, jobId, verdict, reason = null, stage, configHash }) {
  db.prepare(
    `INSERT INTO listing_verdicts
       (listing_id, job_id, verdict, reason, stage, evidence_kind, evidence_hash, decided_at)
     VALUES (?, ?, ?, ?, ?, 'final', NULL, ?)
  ...
  ).run(listingId, jobId, verdict, verdict === 'rejected' ? reason : null, stage, Date.now());
