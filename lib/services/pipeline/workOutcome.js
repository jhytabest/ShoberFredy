...
import { backoffMs } from '../../shared/async.js';
...
export function parkOutcome(
  ...
  note,
  untilMs,
  { counter = null, providerSignal = null, environmental = false } = {},
) {
  return outcome({
    cause: 'policy',
    disposition: 'park',
...
const BY_KIND = {
  ...
  challenge: (classified, { provider, market, now, parks }) =>
    parkOutcome('challenge', classified.message, now + backoffMs(parks), {
      providerSignal: provider && market ? { provider, market, scope: 'item', signal: 'challenge' } : null,
    }),
  rate_limit: (classified, { now, parks }) =>
    parkOutcome('rate_limit', classified.message, now + (classified.retryAfterMs ?? backoffMs(parks))),
...
export function classifyWorkFailure(spec, item, error, { now = Date.now() } = {}) {
  ...
  return build(classified, {
    ...
    market: item?.payload?.market ?? null,
