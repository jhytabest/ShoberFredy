...
// For work that something else re-enqueues on a schedule: liveness every
// maintenance pass and maintenance on a fresh key each interval. Retrying
// inside the item only duplicates the recovery the schedule already provides.
...
// `environmental` marks a wait the item did nothing to cause and cannot resolve
// by being retried later — no proxy configured, provider paused. Counting those
// against the deferral budget abandons healthy work for an outage somewhere
// else: a proxy absent for longer than MAX_DEFERRALS * PROXY_WAIT_MS killed
// every immowelt liveness probe outright, so those listings could never be
// checked again. The age cap still applies and still guarantees termination.
export function deferWork(
  ...
  return SqliteConnection.withTransaction((db) => {
    ...
    if (parkedOut || age > env('FREDY_WORK_MAX_PARK_MS')) {
      terminate(db, kind, key, 'dead', {
        outcome: 'abandoned',
        code: 'parked_out',
