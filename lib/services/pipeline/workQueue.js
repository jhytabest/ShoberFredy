...
async function superviseLoop(spec) {
  while (true) {
    try {
      ...
      logger.error(`${spec.name} worker loop stopped unexpectedly; restarting.`, error);
...
async function drain(spec) {
  while (true) {
    ...
    try {
      ...
      logger.error(`${spec.name} queue claim failed; retrying.`, error);
    ...
    try {
      ...
      try {
        ...
        logger.error(`${spec.name} worker failed to record a queue failure; continuing.`, recoveryError);
        await delay(env('FREDY_WORK_IDLE_POLL_MS'));
      }
    }
  }
