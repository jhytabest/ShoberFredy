...
import { recordProviderSignal } from '../jobs/providerCircuitBreaker.js';
...
// Maintenance receives a fresh key each interval; the schedule provides recovery.
...
export function enqueueWork(kind, key, payload, { mode = 'reset', fingerprintKey = null, merge = null } = {}) {
  return SqliteConnection.withTransaction((db) => {
    ...
    if (!existing) {
      ...
      writeAudit(db, kind, key, { action: 'enqueued' });
    ...
    for (const stale of ['llmAttempts', 'geocodeAttempts', 'completedAt']) delete merged[stale];
    db.prepare(
      `UPDATE pipeline_work
       SET payload_json = @payload, status = 'pending', attempt_count = 0, lease_until = NULL,
           next_attempt_at = 0, last_error = NULL, defer_count = 0,
           outcome = NULL, outcome_code = NULL, outcome_note = NULL, updated_at = @now
       WHERE kind = @kind AND key = @key`,
    ).run({ kind, key, payload: toJson(merged) ?? '{}', now });
    writeAudit(db, kind, key, {
      action: 'reset',
      payload: {
        previousStatus: existing.status,
        previousOutcome: existing.outcome,
        previousAttempts: existing.attempt_count,
      },
    });
    return { key, changed: true, status: 'pending' };
  });
      ...
      .prepare(
        `SELECT * FROM pipeline_work w
         WHERE w.kind = @kind AND w.status IN ${CLAIMABLE_SQL}
           ...
           ${filter?.sql ?? ''}
         ORDER BY CASE WHEN w.status = 'processing' THEN 0
                       WHEN w.created_at < @now - 3600000 THEN 1 ELSE 2 END,
                  CASE WHEN w.created_at < @now - 3600000 THEN w.created_at END ASC,
                  w.created_at ${newestFirst ? 'DESC' : 'ASC'}, w.key
         LIMIT 1`,
      )
...
function takeClaim(db, kind, row, { now, leaseMs }) {
  ...
  writeAudit(db, kind, row.key, {
    action: reclaimed ? 'reclaimed' : 'claimed',
    reason: reclaimed ? 'Previous lease expired' : null,
  });
...
export function retryWork(
  ...
  return SqliteConnection.withTransaction((db) => {
    ...
    if (!row || !CLAIMABLE_STATUSES.includes(row.status)) return { status: row?.status ?? 'unchanged', attempts: 0 };
...
// Resource outages defer durable work without consuming its semantic retry budget.
export function deferWork(
  kind,
  key,
  reason,
  untilMs,
  { counter = null, code = 'transient', environmental = false } = {},
) {
  return SqliteConnection.withTransaction((db) => {
    const row = selectRow(db, kind, key);
    ...
    const parkedOut = !environmental && parks > env('FREDY_WORK_MAX_DEFERRALS');
    if (parkedOut || (!environmental && age > env('FREDY_WORK_MAX_PARK_MS'))) {
      terminate(db, kind, key, 'dead', {
        outcome: 'abandoned',
        code: 'parked_out',
...
export function applyOutcome(kind, key, decision) {
  ...
  if (decision.disposition === 'park') {
    return deferWork(kind, key, decision.note ?? decision.code, decision.untilMs ?? Date.now(), {
      ...
      code: decision.code,
      environmental: decision.environmental === true,
    });
  }
...
async function drain(spec) {
  while (true) {
    ...
    try {
      await withOperationDeadline((signal) => spec.handler(item, { signal }), {
        name: `${spec.name}:${item.key}`,
