...
export function failWork(kind, item, error) {
  ...
  // The severity travels with the status because the caller cannot infer it. A
  // provider answering "this advert is gone" arrives here as a thrown
  // ProviderInactiveError and settles as `done`/`inactive`: it is the answer to
  // the question the probe asked, and reporting it as a failed item — which is
  // what a single unconditional warn did, 84 times in six hours, each with a
  // stack — buries the failures that are real.
...
export function completeWork(
  ...
  // `status` has always named the domain outcome — 'inactive', 'sent',
  // 'waiting_model' — and it now lands in the column of that name. The lifecycle
  // column gets 'done', which is the honest answer to "where is this item".
...
export function cancelWork(
  ...
  SqliteConnection.withTransaction((db) => {
    // The reason is a policy sentence or a filter code, never an exception, so it
    // goes to outcome_note and last_error stays empty. Reading 'blacklist' out of
    // last_error is what made a queue of healthy refusals look like a queue of
    // errors.
...
/**
 * Claim a batch of due items that belong together, all under one lease.
 *
 * Notification is the one kind whose unit of work is a group rather than an
 * item: a discovery run finishes rating a burst of listings at once and they go
 * out as a single digest per job and provider. It used to escape the queue
 * entirely for that reason — a bespoke setTimeout loop guarded by a module-level
 * boolean, with no lease, no `processing` transition and no worker registration,
 * so a crash mid-send left the rows claimable with no record of a partial
 * delivery and the health endpoint could not see the dispatcher at all.
 *
 * The oldest due item picks the group; everything else due sharing its
 * `groupBy` payload field joins it.
 *
 * @param {string} kind
 * @param {{limit: number, groupBy: string, now?: number, leaseMs?: number}} options
 * @returns {object[]} hydrated items, all leased; empty when nothing is due
 */
export function claimWorkBatch(kind, { limit, groupBy, now = Date.now(), leaseMs = DEFAULT_LEASE_MS }) {
  ...
  return SqliteConnection.withTransaction((db) => {
    ...
    for (const row of batch) {
      ...
      if (reclaimed) writeAudit(db, kind, row.key, { action: 'reclaimed', reason: 'Previous lease expired' });
      claimed.push(hydrate(selectRow(db, kind, row.key)));
    }
    return claimed;
  });
}
...
function terminate(
  ...
  // All three vocabularies are closed, and checked here because this is the one
  // place that writes them. With no test harness in this repo, a write that
  // throws the moment something invents a value is the only thing keeping the
  // metrics meaning what they say.
  ...
  const changed = db
    ...
    .run({
      ...
      key,
      status,
      // Only an exception belongs here. A policy sentence or a filter code is not
      // an error, and reading one out of this column is how the queue came to
      // look like it was failing thousands of times a day.
...
export function auditWork(kind, key, event, db = null) {
  // The connection is last and optional, which is exactly the shape that invites
  // a caller to pass it first. One did, and every argument shifted: the event
  // object arrived where the database belongs, `db.prepare` was not a function,
  // and the throw took down the surrounding discovery transaction — silently
  // dropping the card it was auditing. A positional optional argument cannot
  // enforce its own order, so it asserts instead of trusting.
...
/** One claim result for the loop, whether the kind batches or not. */
  ...
  while (true) {
    ...
    try {
      ...
      try {
        ...
        // A batch fails as a batch: every member was claimed together under one
        // lease, so every member has to reach an outcome or the rest stay leased
        // until it expires.
        ...
        // A settled outcome is not a failure and carries no stack worth printing.
