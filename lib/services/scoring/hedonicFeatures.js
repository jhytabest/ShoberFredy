...
export const HEDONIC_TERMS = [
  ...
  { name: 'log_size_sq', value: (l) => Math.log(l.size / BASELINE_SIZE_SQM) ** 2 },
  { name: 'rooms_dev', value: (l) => (l.rooms == null ? 0 : clamp(l.rooms, 1, 6) - BASELINE_ROOMS) },
  {
    name: 'rooms_per_70sqm',
    value: (l) => (l.rooms == null ? 0 : (clamp(l.rooms, 1, 6) * BASELINE_SIZE_SQM) / l.size - BASELINE_ROOMS),
  },
  ...
  {
    name: 'year_recent',
    value: (l) => (l.buildingYear == null ? 0 : Math.max(0, clamp(l.buildingYear, 1870, 2026) - 2000) / 25),
  },
