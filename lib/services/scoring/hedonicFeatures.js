 ...
 *
 * Both models train on the stated Kaltmiete and nothing else (see corpus.js),
 * so the rent basis is not a feature at all: there is only one basis in the
 * corpus. A 'price_cold_est' flag used to mark rows whose cold rent had been
 * imputed from warm minus charges, so each model could absorb the imputation
 * bias; there is no imputation left to absorb.
 */
...
/**
 * Ranked worst to best, and keyed on the extraction's own vocabulary.
 *
 * It was not. `well_kept` and `modernised` are not values `CONDITIONS` can
 * produce, so those two rungs were unreachable, while `well_maintained` and
 * `first_occupancy_after_renovation` — both real answers — matched nothing and
 * fell through to 0, the code for "the extraction could not tell". Two of the
 * seven conditions a listing can actually have were scored as no condition at all.
 */
...
/*
 * No lease_fixed/lease_sublet. `lease_type` was optional until now and came back
 * 'unknown' on 76% of extractions, so across the existing corpus both flags are
 * very nearly constant false — a column carrying no signal. Now that the field is
 * required and described, new extractions will populate it properly, which is
 * worse rather than better: the model would be trained on rows where the flag
 * means "nobody asked" and would score rows where it means "not a sublet".
 * Reintroduce them once the corpus has turned over.
 */
...
export function structuredFeatureFlags(attrs = {}, freeText = '') {
  ...
  // `first_occupancy_after_renovation` is the most renovated a flat can be short
  // of new-build, and it was the one value missing from this list.
  const renovated = [
    'first_occupancy',
 ...
 *   propertyType, features (from structuredFeatureFlags).
 * @returns {number[]}
 */
 ...
 *
 * @param {object} listing needs: size, rooms, floor, buildingYear,
 *   propertyType, latitude, longitude, geocodeQuality,
