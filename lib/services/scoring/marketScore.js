...
// rebuilt once per retrain rather than per listing, and kept per market because
// each city has its own artifact of each family.
function loadCachedModel(db, family, market) {
  const slot = `${family}|${market}`;
  const stamp = db
    .prepare(`SELECT created_at FROM homeserver_models WHERE family = ? AND market = ?`)
    .get(family, market);
  if (!stamp) {
    cache.delete(slot);
    return null;
  }
  if (cache.get(slot)?.createdAt !== stamp.created_at) {
    let entry = loadModel(db, family, market);
    let prepared = null;
    ...
    cache.set(slot, { createdAt: stamp.created_at, entry, prepared });
  }
  return cache.get(slot);
}

function geocodeAccuracy(db, address, market) {
...
export function scoreListingNow(listing, precomputedAttrs = null) {
  ...
  // A listing nobody could place in a city has no market to be priced against.
  // Leaving it unscored is honest; pricing it off another city's model is not.
  const market = listing.market || null;
  ...
  const accuracy = geocodeAccuracy(db, listing.address, market);
  ...
  let gbm = null;
  try {
    ridge = scoreRidge(loadCachedModel(db, RIDGE_FAMILY, market), input);
  } catch (error) {
    logger.warn('ridge scoring failed for a listing; continuing without it', error);
  }
  try {
    gbm = scoreGbm(loadCachedModel(db, GBM_FAMILY, market), input);
  } catch (error) {
