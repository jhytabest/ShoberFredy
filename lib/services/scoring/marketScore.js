 ...
 * listing cannot be scored at all: no Kaltmiete, missing size, no artifacts, or
 * both models out of band.
 *
 ...
 * @returns {{
 *   actualPricePerSqm: number, swap: boolean,
 *   coordQuality: string,
...
export function scoreListingNow(listing, precomputedAttrs = null) {
  ...
  // Both models are fitted on Kaltmiete, so only a Kaltmiete can be compared with
  // them. A warm ask used to be scored anyway, against a cold fair price, with a
  // label on the message saying so — an advert quoting €1,200 warm was reported
  // as far over the market when it may well have been under it. An unanswerable
  // question is better left unanswered than answered on the wrong basis.
