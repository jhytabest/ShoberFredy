...
/**
 * Check whether a listing is already known WITHOUT mutating the cache.
 *
 * Use this inside retryable operations (the durable parsing pipeline) and
 * commit via {@link addEntry} only after the listing was actually stored —
 * otherwise a retry after a mid-operation failure would see its own hash
 * and misclassify the listing as a duplicate.
 *
 * @param {Object} params - Listing fields
 * @param {string|undefined|null} params.title - The listing title
 * @param {string|undefined|null} params.address - The listing address
 * @param {number|string|undefined|null} params.price - The listing price
 * @returns {boolean} true when the entry is already in the cache
 */
export const hasEntry = ({ title, address, price }) => {
  return cache.has(toHash(title, price, address));
};

/**
 * Add a listing to the cache. Counterpart of {@link hasEntry}; call it after
 * the listing was durably stored.
 *
 * @param {Object} params - Listing fields
 * @param {string|undefined|null} params.title - The listing title
 * @param {string|undefined|null} params.address - The listing address
 ...
 * @returns {void}
 */
export const addEntry = ({ title, address, price }) => {
  cache.add(toHash(title, price, address));
};
