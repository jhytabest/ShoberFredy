...
/**
 * Add a listing to the cache after it was durably stored.
 *
 ...
 * but the old hash would still be found here and the pipeline
 ...
 * the cache's best-effort design: the next stored listing re-adds the hash and
 * the hourly reload rebuilds it from storage. The only consequence is
 * that a genuine duplicate may slip through once, which is far less harmful than
 * a hard-deleted listing never reappearing.
 *
 * Uses the cache's standard hashing rules (null/undefined ignored,
 * falsy-but-valid values like 0 preserved).
 *
