...
export const upsertJob = ({
  ...
  if (existing) {
    ...
    SqliteConnection.execute(
      `INSERT INTO jobs (id, user_id, enabled, name, blacklist, provider, notification_adapter, spatial_filter, spec_filter)
       VALUES (@id, @user_id, @enabled, @name, @blacklist, @provider, @notification_adapter, @spatialFilter, @specFilter)`,
      {
        id,
        user_id: ownerId,
        enabled: enabled ? 1 : 0,
...
export const removeJob = (jobId) => {
  SqliteConnection.withTransaction((db) => {
    // Deleting the job takes its verdicts and its sources with it, by cascade.
    // The listings survive: a listing is global now, one advert is usually
    // wanted by more than one search, and the market model is a second consumer
    // that has nothing to do with which searches happen to be configured today.
    // Deleting a job used to delete every listing it had ever found, which threw
    // away training data on a configuration change.
    db.prepare(`DELETE FROM jobs WHERE id = ?`).run(jobId);
    // What is left with no verdict and no source belongs to nobody at all.
    db.prepare(
 ...
 * geocoding pipeline, tracker, and dashboard operate on active jobs only. The UI,
