...
export const upsertJob = ({
  ...
  if (existing) {
    ...
    SqliteConnection.execute(
      `INSERT INTO jobs (id, user_id, enabled, name, provider, notification_adapter, spatial_filter, spec_filter)
       VALUES (@id, @user_id, @enabled, @name, @provider, @notification_adapter, @spatialFilter, @specFilter)`,
      {
        id,
        user_id: ownerId,
        enabled: enabled ? 1 : 0,
