...
export const createJob = (document) => {
  ...
  SqliteConnection.execute(
    `INSERT INTO jobs (id, enabled, name, city, blacklist, intent_filter, provider,
                       notification_adapter, spatial_filter, spec_filter)
     VALUES (@id, @enabled, @name, @city, @blacklist, @intentFilter, @provider,
             @notificationAdapter, @spatialFilter, @specFilter)`,
...
function storedColumns(document) {
  ...
  if ('notificationAdapter' in document) stored.notificationAdapter = toJson(document.notificationAdapter ?? []);
