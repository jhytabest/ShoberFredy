 ...
 *   link?: string,
 *   hidden_reason?: string|null, // 'blacklist' | 'spec_filter' | 'area_filter' | 'no_coordinates'
  ...
  SqliteConnection.withTransaction((db) => {
    const stmt = db.prepare(
      `INSERT INTO listings (id, hash, provider, job_id, price, size, rooms, title, image_url, description, address,
                             ...
                             canonical_schema_version, source_urls_json)
    ...
    const attrsStmt = db.prepare(
      `INSERT OR REPLACE INTO listing_attributes (
         ...
         heating_type, energy_value_kwh, amenities_json, availability, comments, schema_version,
         ...
         @heatingType, @energyValueKwh, @amenitiesJson, @availability, @comments, @schemaVersion,
    ...
    for (const item of listings) {
      const params = {
        ...
        canonical_schema_version: item.canonicalSchemaVersion ?? 0,
 ...
 * @returns {void}
 */
export const resetDistancesForUser = (userId) => {
  // Coordinates stay: they come from the persistent geocode cache, not from
  // the home address. Only the distances depend on the home address.
  SqliteConnection.execute(
    `UPDATE listings
     SET distance_to_destination = NULL
     WHERE job_id IN (
