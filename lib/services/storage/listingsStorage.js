 ...
 * - Structured attributes (item.attributes, from parseListingAttrs) and the
 ...
 *   attributes?: object,   // parseListingAttrs output (+ features flags)
    ...
    for (const item of listings) {
      ...
      if (item.marketScore) {
        ...
        scoreStmt.run({
          listingId: params.id,
          scoredAt: Date.now(),
          modelCreatedAt: s.modelCreatedAt ?? null,
          actualPricePerSqm: s.actualPricePerSqm,
          fairPricePerSqm: s.fairPricePerSqm,
          deltaPercent: s.deltaPercent,
          zScore: s.zScore,
          confidence: s.confidence,
          comps500m: s.comps500m,
