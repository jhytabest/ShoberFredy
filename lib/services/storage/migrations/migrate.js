...
import { isCorrectedSourceIdentityUpgrade, isLegacyBaselineUpgrade } from './legacyBaseline.js';
...
export async function runMigrations() {
  ...
  for (const m of files) {
    ...
    const legacyBaseline = isLegacyBaselineUpgrade(m.name, checksum, executed);
    const correctedSourceIdentity = isCorrectedSourceIdentityUpgrade(m.name, checksum, executed);
    if (executed.has(m.name) && !legacyBaseline && !correctedSourceIdentity) {
      throw new Error(`Applied migration ${m.name} has changed; add a new migration instead.`);
    }
    ...
    if (correctedSourceIdentity) {
      SqliteConnection.withTransaction((db) => {
        db.prepare(
          `INSERT INTO pipeline_audit_events (stage, action, payload_json, created_at)
          VALUES ('migration', 'corrected_checksum_advanced', ?, ?)`,
        ).run(JSON.stringify({ name: m.name, previousChecksum: executed.get(m.name), checksum }), Date.now());
        db.prepare(
          `UPDATE schema_migrations SET checksum = ?, applied_at = datetime('now'), duration_ms = 0
          WHERE name = ?`,
        ).run(checksum, m.name);
      });
      executed.set(m.name, checksum);
      logger.info(`Migration checksum advanced: ${m.name}`);
      continue;
    }
    ...
    try {
      let duration = 0;
      SqliteConnection.withTransaction((db) => {
        const previous = legacyBaseline
          ? db
              .prepare('SELECT name, checksum, applied_at, duration_ms FROM schema_migrations WHERE name = ?')
              .get(m.name)
          : null;
        const t0 = Date.now();
        fn(db);
        ...
        if (legacyBaseline) {
          db.prepare(
            `INSERT INTO pipeline_audit_events (stage, action, payload_json, created_at)
            VALUES ('migration', 'legacy_baseline_advanced', ?, ?)`,
          ).run(JSON.stringify({ previous, checksum, durationMs: duration }), Date.now());
          db.prepare(
            `UPDATE schema_migrations SET checksum = ?, applied_at = datetime('now'), duration_ms = ?
            WHERE name = ?`,
          ).run(checksum, duration, m.name);
        } else {
          db.prepare(
