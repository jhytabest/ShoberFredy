...
export async function runMigrations() {
  ...
  for (const m of files) {
    ...
    // Foreign keys are enforced everywhere else and deliberately not here.
    // Reaching the current schema from an older one drops the listing corpus
    // wholesale, and dropping parents before children with enforcement on either
    // fails or cascades depending on the order — neither of which says anything
    // useful about a table that is about to be recreated empty. The pragma has to
    // be set out here: inside a transaction it is a silent no-op. Nothing else is
    // running yet, because migrations complete before any worker starts.
