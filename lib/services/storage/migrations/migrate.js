 ...
 *
 * Executes the single current-schema migration. Its checksum is the schema
 * version: when that file changes, the idempotent reconciler runs again and
 * replaces the one-row ledger entry. Historical migration files are never
 * accumulated.
 *
 * Usage:
...
/**
 * Compare a database's migration ledger with the migrations shipped by this
 * build. This is used by both the HTTP health endpoint and the read-only
 * Prometheus exporter, so a reachable SQLite file cannot masquerade as a
 * current schema.
 *
 * @param {import('better-sqlite3').Database} [db]
 * @returns {{upToDate:boolean, appliedCount:number, expectedCount:number, latestApplied:string|null, latestExpected:string|null, missing:string[]}}
 */
...
export async function runMigrations() {
  ...
  for (const m of files) {
    ...
    // Foreign keys are enforced everywhere else and deliberately not here, for
    // the whole migration. This is SQLite's own documented procedure for a table
    // whose shape has to change: a constraint cannot be altered away, so the
    // table is rebuilt — created new, copied, dropped, renamed — and every one of
    // those steps would otherwise either cascade into the children or silently
    // repoint their REFERENCES clauses at the temporary copy. Renaming `listings`
    // with enforcement on empties `listing_images`, and scheduled maintenance
    // then deletes every image file on disk, because an image no row references
    // is an orphan.
    //
    // The pragma has to be set out here: inside a transaction it is a silent
    // no-op. Nothing else is running yet — migrations complete before any worker
    // starts — and the migration ends by asserting referential integrity itself,
    // which is a stronger check than the one being suspended.
