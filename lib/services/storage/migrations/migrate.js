...
export function getMigrationStatus(db = SqliteConnection.getConnection()) {
  ...
  const latestApplied = [...expected].reverse().find((name) => applied.has(name)) ?? null;
...
/**
 * Whether the operator has opted in to rewriting the recorded checksum of an
 * already-applied migration (`yarn migratedb:overwrite`).
 * @returns {boolean}
 */
function checksumUpdateAllowed() {
  return /^(1|true|yes)$/i.test(process.env.MIGRATION_ALLOW_CHECKSUM_UPDATE || '');
}
...
export async function runMigrations() {
  ...
  for (const m of files) {
    ...
    if (executed.has(m.name)) {
      ...
      if (prev !== checksum) {
        // An applied migration whose file changed means the schema on disk no
        // longer describes the schema in the database. Accepting that silently
        // (as this once did) lets the two drift apart undetected, so refuse
        // unless the operator explicitly opts in via migratedb:overwrite.
        if (!checksumUpdateAllowed()) {
          logger.error(
            `Migration ${m.name} was already applied but its file has changed ` +
              `(recorded ${prev.slice(0, 12)}…, found ${checksum.slice(0, 12)}…). ` +
              'Add a new migration instead of editing an applied one, or re-run with ' +
              'MIGRATION_ALLOW_CHECKSUM_UPDATE=true (yarn migratedb:overwrite) if the ' +
              'edit is known to be schema-neutral.',
          );
          process.exitCode = 1;
          return;
        }
        logger.warn(`Mismatch found in migration ${m.name}. Updating recorded checksum on operator request.`);
