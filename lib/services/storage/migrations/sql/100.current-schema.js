 ...
 *
 * `listings.price` was just rewritten onto a cold basis, so every stored
 ...
 *
 * About 248 listings carry no cold-equivalent: most state a Warmmiete with no
 * Nebenkosten to subtract, and a handful state neither figure. Under the old
 * schema nothing asked the model for both — it recorded whichever the headline
 * quoted — so these are an artefact of the question, not of the adverts. The new
 * schema asks for both explicitly and tells the model to derive one from the
 * other, which is worth one call each to find out.
 *
 * A listing with no price is refused for `no_price`, never notified, and is not
 * trainable, so this is also the cheapest way to grow the corpus.
 *
