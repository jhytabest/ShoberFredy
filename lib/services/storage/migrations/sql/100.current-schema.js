...
export function up(db) {
  ...
  const upgrading = tableExists(db, 'listings');

  createCoreTables(db);
  seedSettings(db);
  if (upgrading && columnExists(db, 'parsing_queue', 'queue_kind')) {
...
function createCoreTables(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS listing_attributes (
      ...
      amenities_absent_json TEXT NOT NULL DEFAULT '[]',
      rent_inclusions_json TEXT NOT NULL DEFAULT '[]',
      requirements_json TEXT NOT NULL DEFAULT '[]',
      conflicts_json TEXT NOT NULL DEFAULT '[]',
      recurring_costs_json TEXT NOT NULL DEFAULT '{}',
      one_time_buyout_eur REAL,
      summary TEXT
    );
    ...
    CREATE TABLE IF NOT EXISTS parsing_queue (
      ...
      status TEXT NOT NULL DEFAULT 'pending'
        CHECK(status IN ('pending','processing','retry','completed','duplicate','dead','cancelled')),
    ...
    CREATE TABLE IF NOT EXISTS listing_images (
      id TEXT PRIMARY KEY,
      queue_id TEXT NOT NULL REFERENCES parsing_queue(id) ON DELETE CASCADE,
      listing_id TEXT REFERENCES listings(id) ON DELETE CASCADE,
    ...
    CREATE TABLE IF NOT EXISTS listing_extractions (
      queue_id TEXT PRIMARY KEY REFERENCES parsing_queue(id) ON DELETE CASCADE,
      listing_id TEXT REFERENCES listings(id) ON DELETE CASCADE,
      visual_json TEXT,
      llm_json TEXT,
      vision_model TEXT,
    ...
    CREATE TABLE IF NOT EXISTS llm_call_audit (
      id TEXT PRIMARY KEY,
      queue_id TEXT REFERENCES parsing_queue(id) ON DELETE SET NULL,
    ...
    CREATE TABLE IF NOT EXISTS detail_fetch_queue (
      ...
      next_attempt_at INTEGER NOT NULL DEFAULT 0,
      last_error TEXT,
      capture_queue_id TEXT REFERENCES parsing_queue(id) ON DELETE SET NULL,
      created_at INTEGER NOT NULL,
      updated_at INTEGER NOT NULL,
      completed_at INTEGER,
      UNIQUE(job_id, provider, source_key)
    ...
    CREATE TABLE IF NOT EXISTS listing_sources (
      ...
      source_key TEXT NOT NULL,
      source_url TEXT NOT NULL,
      detail_queue_id TEXT REFERENCES detail_fetch_queue(id) ON DELETE SET NULL,
      parsing_queue_id TEXT REFERENCES parsing_queue(id) ON DELETE SET NULL,
      listing_id TEXT REFERENCES listings(id) ON DELETE SET NULL,
...
function createIndexes(db) {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_parsing_queue_claim
      ON parsing_queue(status, next_attempt_at, lease_until, discovered_at);
    CREATE INDEX IF NOT EXISTS idx_parsing_queue_source
      ON parsing_queue(job_id, provider, source_key, status);
    ...
    CREATE INDEX IF NOT EXISTS idx_processing_attempts_queue ON processing_attempts(queue_id, started_at);
    ...
    CREATE INDEX IF NOT EXISTS idx_detail_fetch_due
      ON detail_fetch_queue(status, next_attempt_at, lease_until, created_at);
    CREATE INDEX IF NOT EXISTS idx_detail_fetch_url ON detail_fetch_queue(provider, source_url);
    ...
    CREATE INDEX IF NOT EXISTS idx_rating_queue_due ON rating_queue(status, next_attempt_at, lease_until);
