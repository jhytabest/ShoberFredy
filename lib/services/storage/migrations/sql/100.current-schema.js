...
export function up(db) {
  ...
  discardStaleScores(db);
  requeueUnpricedListings(db);
}
...
/**
 * Blacklist terms stop being a per-job column and become one setting, pruned to
 * the terms that can actually refuse something.
 *
 * Sixteen of the twenty-seven configured terms had never matched anything in
 * 6,012 recorded refusals, and they could not: `Tausch` and `Tauschwohnung`
 * reduce to the same stem, as do `möbliert` and `Vollmöbliert`, and `furnished`
 * and `partly furnished` — the matcher stems these, so the longer spelling is
 * covered by the shorter one. Six more are misspellings of words the correct
 * spelling already stems over (`möbeliert`, `teilmöbeliert`, `mobiliert` and
 * friends), so they read as coverage while matching nothing. `Juni/Juli` is a
 * date that has passed, and `Tatsächliche Wohnung` is a phrase no advert writes.
 *
 * Pruning is safe precisely because these terms are inert: the stems that remain
 * accept everything the removed ones would have. What it buys is a list an
 * operator can read and reason about, instead of one that looks like it refuses
 * twenty-seven things and refuses eleven.
 */
function migrateBlacklistToSettings(db) {
  if (!tableExists(db, 'jobs') || !columnExists(db, 'jobs', 'blacklist')) return;
  // Terms the stemming matcher already covers via a shorter form, or that cannot
  // match German advert text at all. Compared case-insensitively.
  const INERT = new Set(
    [
      'Tauschwohnung',
      'Wohnungsswap',
  ...
  for (const row of db.prepare(`SELECT blacklist FROM jobs`).all()) {
    ...
    for (const term of Array.isArray(terms) ? terms : []) {
      ...
      if (text && !INERT.has(key) && !seen.has(key)) seen.set(key, text);
    }
  }
  if (seen.size) {
    db.prepare(
...
function migrateAttributeDocuments(db) {
  ...
  const DROPPED = [
    ...
    'sizeSqm',
    'rooms',
    // Extracted only to convert a Warmmiete into a Kaltmiete. That conversion is
    // gone, and nothing else ever read either figure.
    'serviceChargesEur',
    'heatingCostsEur',
  ];
  const DROPPED_LLM = [
  ...
  const DROPPED_RENT = [
    ...
    'service_charges',
    'heating_costs',
    // The basis is read off `cold` and `warm` now, not asked for a third time.
    'price_type',
  ];
  ...
  // The same rule as buildCanonical in lib/services/pipeline/listingFinalizer.js,
  // restated rather than imported: a migration runs before the application is
  // wired up and must not depend on it. Cold when the advert stated one, warm
  // otherwise, and no conversion between them.
  const basisOf = (a) => {
    if (a.coldRentEur > 0) return { rent: a.coldRentEur, type: 'cold' };
    if (a.warmRentEur > 0) return { rent: a.warmRentEur, type: 'warm' };
    return { rent: null, type: 'unknown' };
  };
  ...
  // Every score is now a Kaltmiete against a Kaltmiete model, so there is no
  // basis to record. discardStaleScores clears the rows either way.
  if (columnExists(db, 'homeserver_listing_model_scores', 'price_type')) {
    db.exec(`ALTER TABLE homeserver_listing_model_scores DROP COLUMN price_type`);
  }
  ...
  if (rewritten) {
    ...
    console.log(`Rewrote ${rewritten} attribute documents; ${priced} carry a rent, ${unpriceable} do not.`);
  }
}

/**
 * Throw away every stored market score.
 *
 * `listings.price` was just rewritten from the stated rents, so every stored
 * `actual_price_per_sqm` was computed from a number that no longer exists — and
 * the notification renders those figures. They are recomputed by the next rating
 * pass from the price that does exist, and the artifacts are feature-space stale
 * anyway, so there is nothing here worth keeping.
 */
function discardStaleScores(db) {
  if (!tableExists(db, 'homeserver_listing_model_scores')) return;
  const cleared = db.prepare(`DELETE FROM homeserver_listing_model_scores`).run().changes;
  // eslint-disable-next-line no-console
  if (cleared) console.log(`Cleared ${cleared} market score(s) priced on the old basis.`);
}

/**
 * Buy one more extraction for the listings that ended up with no rent at all.
 *
 * The old schema asked for whichever figure the headline quoted and recorded that
 * one, so a listing stored with no rent at all may well have had one on the page.
 * The new schema asks for the Kaltmiete and the Warmmiete as separate questions,
 * which is worth one call each to find out.
 *
 * A listing with no price is refused for `no_price`, never notified, and cannot
 * be trained on, so this is also the cheapest way to grow the corpus.
 *
 * Once only, guarded by a marker in `pipeline_control` — which survives a corpus
 * discard — so re-running the migration cannot re-buy them.
 */
function requeueUnpricedListings(db) {
  if (!tableExists(db, 'pipeline_control') || !tableExists(db, 'listing_sources')) return;
  const done = db.prepare(`SELECT 1 FROM pipeline_control WHERE name = 'repriced_unpriced_listings'`).get();
  if (done) return;
  ...
  const requeued = db
    .prepare(
      `INSERT INTO pipeline_work (kind, key, payload_json, status, created_at, updated_at)
       SELECT 'detail',
              s.detail_queue_id,
              json_object('provider', s.provider, 'sourceKey', s.source_key, 'sourceUrl', s.source_url),
              'pending', @now, @now
       FROM listings l
       JOIN listing_sources s ON s.listing_id = l.id AND s.detail_queue_id IS NOT NULL
       WHERE l.price IS NULL AND l.state = 'active'
       GROUP BY l.id
       ON CONFLICT(kind, key) DO UPDATE SET
         status = 'pending', attempt_count = 0, lease_until = NULL, next_attempt_at = 0,
         last_error = NULL, outcome = NULL, outcome_code = NULL, outcome_note = NULL,
         defer_count = 0, updated_at = excluded.updated_at`,
    )
    .run({ now }).changes;

  db.prepare(
    `INSERT INTO pipeline_control (name, value, updated_at) VALUES ('repriced_unpriced_listings', @value, @now)
     ON CONFLICT(name) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`,
  ).run({ value: String(requeued), now });
  // eslint-disable-next-line no-console
  if (requeued) console.log(`Requeued ${requeued} listing(s) with no rent for one more extraction.`);
}
