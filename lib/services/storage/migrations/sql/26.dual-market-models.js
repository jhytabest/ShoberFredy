 ...
 * - parser-derived warm-minus-charges values are cleared from cold_rent_eur;
 *   the shared target builder reconstructs and labels them as 'cold_est'.
 *
...
export function up(db) {
  ...
  const hasListingAttributes = db
    .prepare(`SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'listing_attributes'`)
    .get();
  if (hasListingAttributes) {
    db.exec(`
      UPDATE listing_attributes
      SET cold_rent_eur = NULL
      WHERE price_type = 'warm'
        AND cold_rent_eur IS NOT NULL
        AND warm_rent_eur IS NOT NULL
        AND service_charges_eur IS NOT NULL
        AND ABS(cold_rent_eur - (warm_rent_eur - service_charges_eur)) < 0.01
    `);
  }
}
