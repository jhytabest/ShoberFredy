...
/**
 * The blacklist terms, which are deployment-wide rather than per job.
 *
 * They used to be a column on `jobs`, and three jobs meant three lists that
 * agreed only by hand. They had already drifted by one term. Divergence there is
 * not merely untidy: work is keyed by advert while the card filter runs per job,
 * so a term present on two jobs and absent on the third still buys the page
 * fetch and the LLM call the other two refused. One list decides once.
 *
 * Read synchronously and deliberately. `getSettings` is async only because it
 * merges the on-disk deployment config, and every consumer of these terms — the
 * card filter, the intent rules, the configuration hash — sits on a synchronous
 * path that would have to become async to reach it.
 *
 * @returns {string[]}
 */
