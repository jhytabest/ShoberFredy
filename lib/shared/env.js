 ...
 *   'int'    positive integer; a non-positive or unparsable value falls back
...
const REGISTRY = {
  ...
  // ---- discovery ----------------------------------------------------------
  FREDY_DISCOVERY_MAX_PAGES: { kind: 'int', default: 20, doc: 'Hard ceiling on pages walked per provider run.' },
  ...
  FREDY_LLM_VISION_MODEL: { kind: 'string', default: '', doc: 'OpenRouter model id for image analysis.' },
  FREDY_LLM_VISION_ENABLED: { kind: 'flag', default: false, doc: 'Set 1 to enable supplemental vision analysis.' },
  FREDY_LLM_VISION_MAX_IMAGES: { kind: 'int', default: 8, doc: 'Images sent per vision request.' },
  ...
  // ---- filters / geocoding ------------------------------------------------
  FREDY_PRELLM_AREA_MIN_PRECISION: {
    kind: 'string',
    default: 'house,street',
    doc: 'Geocode precisions confident enough to reject a listing before the LLM.',
  },
  FREDY_GEOCODER_RETRY_COARSE_AFTER_DAYS: { kind: 'int', default: 14, doc: 'Age at which a coarse geocode retries.' },

  // ---- provider circuit breaker ------------------------------------------
  FREDY_PROVIDER_BREAKER_FAILURES: { kind: 'int', default: 2, doc: 'Failures before a provider is paused.' },
  ...
  FREDY_MARKET_EXPORTER_PORT: { kind: 'int', default: 9217, doc: 'Prometheus exporter port; 0 disables it.' },
  FREDY_MARKET_MODEL_INTERVAL_SECONDS: { kind: 'int', default: 86_400, doc: 'Minimum seconds between retrains.' },
  ...
  FREDY_LIVENESS_RECHECK_DAYS: { kind: 'int', default: 7, doc: 'Age at which a listing is re-captured for liveness.' },
...
export function env(name) {
  const { kind, default: fallback } = spec(name);
