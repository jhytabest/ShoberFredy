 ...
 * Matching is case-insensitive (German locale). Most terms are plain
 * substring matches; a few short German terms get token-aware handling so
 * they stop matching inside unrelated words:
 * - 'wg' only matches as a standalone token ("WG-Zimmer" yes, "Wegweiser"
 *   no) and is ignored in negated phrases ("keine WG").
 * - 'befristet' only matches as a standalone token, so "unbefristet" (the
 *   desirable case) no longer triggers the blacklist.
 * @param {string} word
 * @param {string[]} arr
 * @returns {boolean}
 */
function isOneOf(word, arr) {
  ...
  return arr.some((item) => matchesBlacklistTerm(haystack, item));
}
...
function hasWgToken(haystack) {
  if (!hasToken(haystack, 'wg')) return false;

  const negativeWgPattern = /(^|[^\p{L}\p{N}])(kein|keine|keinen|nicht)\s+wg([^\p{L}\p{N}]|$)/iu;
  if (negativeWgPattern.test(haystack)) return false;
