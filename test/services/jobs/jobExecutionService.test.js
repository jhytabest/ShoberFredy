...
describe('services/jobs/jobExecutionService', () => {
  ...
  async function initService() {
    ...
    vi.doMock(brokerPath, () => ({
      sendToUser: (...args) => calls.sent.push(args),
    }));
    vi.doMock(utilsPath, () => ({
  ...
  it('forwards SSE jobStatus to the account', async () => {
    state.jobsById['j1'] = { id: 'j1', userId: 'owner1' };

    await initService();

    bus.emit('jobs:status', { jobId: 'j1', running: true });

    expect(calls.sent.length, 'sendToUser should be called once').toBe(1);
    const [recipient, event, data] = calls.sent[0];
    expect(event).toBe('jobStatus');
    expect(data).toEqual({ jobId: 'j1', running: true });
    expect(recipient).toBe('owner1');
  });

  it('runs all jobs for the single account', async () => {
    state.jobsList = [
