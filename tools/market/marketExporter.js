 ...
 *
 * Runs as its own process/container with a read-only view of the SQLite file
 * and serves /metrics (Prometheus text format) plus /health.
 *
...
const config = {
  ...
  port: Number.parseInt(process.env.FREDY_MARKET_EXPORTER_PORT || '9217', 10),
};
...
function emitPredictionMetrics(lines) {
  if (!tableExists('homeserver_listing_market_model')) {
    return;
  }

  // Snapshot both queries in one read transaction so a model rewrite
  // committing in between cannot yield a run id whose predictions are gone.
  const snapshot = db.transaction(() => {
    const run = db
      .prepare(
        `
        SELECT id, model_version, training_rows, scored_rows, created_at, metrics_json
  ...
  if (!snapshot) return;
  const { run: latestRun, predictions: rows } = snapshot;
  ...
  metric(lines, 'fredy_market_prediction_mae_eur_per_sqm', Number(runMetrics.medianAbsoluteError));
  ...
  addHeader(
    ...
    'gauge',
    'Out-of-sample model error: median absolute percentage error by evaluation method (naive = predict the global median).',
  );
  for (const method of ['naive', 'holdout', 'spatialCV']) {
    if (Number.isFinite(evaluation[method]?.mdape)) {
      metric(lines, 'fredy_market_model_error_percent', evaluation[method].mdape, { method, stat: 'mdape' });
    }
    if (Number.isFinite(evaluation[method]?.ppe10)) {
      metric(lines, 'fredy_market_model_error_percent', evaluation[method].ppe10, { method, stat: 'ppe10' });
    }
  }
  addHeader(
    lines,
    ...
    'gauge',
    'Unique flats the latest model run trained on, plus excluded swap listings.',
  );
  metric(lines, 'fredy_market_model_training_flats', Number(runMetrics.uniqueFlats) || 0, { kind: 'unique' });
  metric(lines, 'fredy_market_model_training_flats', Number(runMetrics.rawRows) || 0, { kind: 'raw_rows' });
  metric(lines, 'fredy_market_model_training_flats', Number(runMetrics.swapExcluded) || 0, { kind: 'swap_excluded' });
  addHeader(
    lines,
  ...
  metric(lines, 'fredy_market_prediction_scored_listings', rows.length, { scope: 'all_training' });
  metric(lines, 'fredy_market_prediction_scored_listings', visibleRows.length, { scope: 'all_visible' });
  metric(lines, 'fredy_market_prediction_scored_listings', activeRows.length, { scope: 'active_visible' });
  ...
  emitQuantiles(
    lines,
    'fredy_market_prediction_confidence',
  ...
  emitListingGeoMetrics(lines, activeRows);
  emitTopListingMetrics(lines, activeRows);
  emitDeltaHistogram(lines, visibleRows);
  emitHistoryMetrics(lines, rows);
  emitSurfaceMetrics(lines, latestRun.id);
}
 ...
 */
function emitListingGeoMetrics(lines, activeRows) {
  addHeader(
    lines,
 ...
 */
function emitTopListingMetrics(lines, activeRows) {
  addHeader(
    lines,
  ...
  for (const [window, windowMs] of [
    ...
    const candidates = activeRows
      .filter(
        (row) =>
          row.confidence >= 0.3 && Number.isFinite(row.listing_created_at) && now - row.listing_created_at <= windowMs,
      )
    ...
    candidates.forEach((row, index) => {
      metric(lines, 'fredy_market_top_listing', row.delta_percent, {
        ...
        price_per_sqm: String(roundMetric(row.actual_price_per_sqm)),
        fair_per_sqm: String(roundMetric(row.predicted_price_per_sqm)),
        z_score: String(roundMetric(row.z_score)),
        confidence: String(roundMetric(row.confidence)),
      });
    });
  }
}

function emitDeltaHistogram(lines, visibleRows) {
  addHeader(
    lines,
  ...
  for (let i = 0; i <= edges.length; i += 1) {
    ...
    metric(lines, 'fredy_market_delta_distribution', count, { bucket: label, order: String(i).padStart(2, '0') });
  }
}
...
function emitQuantiles(lines, name, values) {
  for (const [label, q] of [
    ['p10', 0.1],
    ['p25', 0.25],
    ['p50', 0.5],
    ...
    metric(lines, name, quantile(values, q) || 0, { quantile: label });
  }
}
