...
import { normalizeAddress } from '../../lib/services/geocoding/address.js';
import { parseListingAttrs } from '../../lib/services/scoring/listingAttrs.js';
import {
  MAX_MONTH_OFFSETS,
  hedonicDesignVector,
  hedonicDimensions,
...
const config = {
  ...
  intervalSeconds: intEnv('FREDY_MARKET_MODEL_INTERVAL_SECONDS', 15 * 60),
...
async function runDaemon() {
  while (true) {
    try {
      ...
      console.error('market model run failed', error);
    }
...
function runOnce() {
  ...
  try {
    ...
    console.error('surface geojson write failed (map layer will be stale)', error);
  }
...
function enrichListing(row, now) {
  ...
  const features = textFeatureFlags(row.title, row.description, address);
